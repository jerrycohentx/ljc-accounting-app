...
import { getReclassHistoryForJournal } from '../lib/journal-reclass-history.js';
...
router.get('/:id', entityAccessMiddleware, async (req, res) => {
  try {
    ...
    // Append-only reclasses (manual Fix category + learned-rule bulk moves).
    // Original lines never change; the UI overlays this history so detail matches
    // the reconcile Category column.
    let reclassHistory = [];
    try {
      reclassHistory = await getReclassHistoryForJournal(db, req.entityId, req.params.id, lines);
    } catch {
      reclassHistory = [];
    }
