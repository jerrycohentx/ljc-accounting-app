...
7. Before claiming books are clean or openings rebuilt: run `node scripts/verify-books-clean.mjs --entity ent-ljc --asOf YYYY-MM-DD` — **BS Net Income must equal calendar YTD P&L**, prior-year P&L closed (`netIncomeTieoutOk`), TB/BS balanced, plugs $0, **CF netCashFlow = BS Δ for all 100x cash** (never compare CF to Simmons 1000 alone). See `.cursor/rules/books-clean-definition-of-done.mdc`.
