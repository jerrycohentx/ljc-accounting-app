...
export default function QBDLayout() {
  ...
  const menuDefs = (n) => {
    switch (n) {
      ...
      case 'Banking': return [['Write Checks', () => nav('/write-checks')], ['Make Deposits', () => nav('/make-deposits')], ['Use Register…', useRegisterFor], ['Reconcile…', () => nav('/reconcile')], '-', ['Activity Review…', () => nav('/feed-review')], ['Import ACH interest JE…', () => nav('/ach-interest-import')], ['Email ingest…', () => setEmailOpen(true)], ['Bank Feeds', () => nav('/bank-feeds')], ['Bank Import (Plaid/OFX)…', () => nav('/bank-import')]];
