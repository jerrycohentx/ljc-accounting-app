' LJC Accounting — local server launcher (developers / troubleshooting only)
Option Explicit
...
If oShell.Run("cmd /c node -v >nul 2>&1", 0, True) <> 0 Then
    MsgBox "Node.js is not installed." & vbCrLf & vbCrLf & _
           "Install from https://nodejs.org then try again.", vbCritical, "LJC Accounting (local)"
    WScript.Quit 1
End If

On Error Resume Next
oShell.Run "mshta javascript:var s=new ActiveXObject('WScript.Shell');s.Popup('Starting local LJC Accounting…',2,'LJC Accounting',64);close()", 0, False
On Error GoTo 0
...
MsgBox "The local server did not start in time." & vbCrLf & vbCrLf & _
       "For errors, open START-APP-Visible.bat" & vbCrLf & _
       "Log: " & sLog, vbCritical, "LJC Accounting (local)"
WScript.Quit 1
