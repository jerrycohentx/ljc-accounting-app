...
import { QBO_PL_KNOWN_ADJUSTMENTS } from '../config/qbo-known-adjustments.js';
...
export function buildProfitLossFromQboTb(rootDir, entityId, { startDate, endDate, companyName }) {
  ...
  for (const adj of QBO_PL_KNOWN_ADJUSTMENTS) {
    if (adj.entityId !== entityId || adj.taxYear !== taxYear) continue;
    const needle = String(adj.accountNameIncludes || '').toLowerCase();
    const row = revenues.find((r) => r.name.toLowerCase().includes(needle));
    if (!row) continue;
    const reduceBy = Number(adj.reduceRevenueBy) || 0;
    if (Math.abs(reduceBy) < 0.005) continue;
    row.amount = round2(row.amount - reduceBy);
    totalRev = totalRev.minus(reduceBy);
    appliedAdjustments.push({
      id: adj.id,
      accountName: row.name,
      reduceRevenueBy: reduceBy,
      amountAfter: row.amount,
      reason: adj.reason,
      offsetAccountNumber: adj.offsetAccountNumber,
    });
    if (Math.abs(row.amount) < 0.005) {
      const idx = revenues.indexOf(row);
      if (idx >= 0) revenues.splice(idx, 1);
    }
  }
  ...
  if (appliedAdjustments.length) {
    sourceNote +=
      ' Known QBO mis-posts have been corrected on this report (see adjustments) — not plugs.';
  }
...
/**
 * Prefer QBO YE TB P&L for a full calendar year when the file exists — even if the
 * app has a small correcting JE — because 2025 detail lives in QBO, not the app GL.
 */
export function shouldPreferQboProfitLoss(entityId, startDate, endDate, appStatement) {
  const year = String(startDate || '').slice(0, 4);
  if (!year || startDate !== `${year}-01-01` || endDate !== `${year}-12-31`) return false;
  const fileName = ENTITY_TB_FILES[entityId];
  if (!fileName || !String(fileName).startsWith(`${year}_`)) return false;
  if (isEmptyAppProfitLoss(appStatement)) return true;
  // App may only have a known correcting JE; still prefer QBO detail for the year.
  const appAbs =
    Math.abs(appStatement?.totals?.totalRevenue || 0) + Math.abs(appStatement?.totals?.totalExpense || 0);
  return appAbs < 500000;
}
