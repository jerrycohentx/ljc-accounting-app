...
async function accountIssue(db, account, session) {
  if (!session) {
    return {
...
export async function getPeriodIntegrityStatus(db, {
  ...
  for (const account of accounts) {
    ...
    for (const month of months) {
      ...
      const issue = await accountIssue(db, account, session);
  ...
  const canClose = reconciliationsOk && !plugsBlocked && netIncomeTieoutOk;
  ...
  return {
    ...
    blockers,
    agentRule:
      'Do not tell Jerry a period is closed unless isClosed===true from this payload. Do not invent status. Before claiming books are clean, BS Net Income must equal YTD P&L (netIncomeTieout).',
  };
}
