...
export async function reclassPostedOffsetLine(db, {
  ...
  // Chain from the latest unreverted correction for this line (manual Fix category
  // or learned-rule reclass) — not always the original posted dump account.
  ...
  const fromAccountId = effective.fromAccountId;

  if (String(target.id) === String(fromAccountId)) {
  ...
  if (existing) {
    const fromExisting = await db.get('SELECT id, account_number FROM accounts WHERE id = ?', [fromAccountId]);
    return {
      alreadyApplied: true,
      reclassJeId: existing.id,
      reclassJeNumber: existing.je_number,
      fromAccountId: fromExisting?.id || fromAccountId,
      fromAccountNumber: fromExisting?.account_number || null,
  ...
  const fromAcct = await db.get('SELECT id, account_number FROM accounts WHERE id = ?', [fromAccountId]);
  if (!fromAcct) throw new Error('Current category account not found');
