...
export async function reclassPostedOffsetLine(db, {
  ...
  // Chain from the latest unreverted reclass for this line (effective category),
  // not always the original posted offset account.
  const prior = await db.get(
    `SELECT id, je_number, memo FROM journal_entries
     WHERE entity_id = ? AND status = 'POSTED' AND reversed_by_je_id IS NULL
       AND memo LIKE ?
     ORDER BY created_at DESC, je_number DESC
     LIMIT 1`,
    [entityId, `reclass-offset:${journalId}:${lineId}:%`]
  );
  let fromAccountId = line.account_id;
  if (prior?.memo) {
    const parts = String(prior.memo).split(':');
    // reclass-offset : journalId : lineId : toAccountId
    if (parts[3]) fromAccountId = parts[3];
  }
