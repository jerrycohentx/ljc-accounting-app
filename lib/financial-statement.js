...
import { computePeriodNetIncome, calendarYearStart } from './net-income-tieout.js';
...
/** Keep zero-balance parents when any descendant has activity (needed for nesting). */
function keepActiveWithAncestors(accounts) {
  const byId = new Map(accounts.map((a) => [a.id, a]));
  const keep = new Set();
  for (const a of accounts) {
    if (calculateAccountBalance(a).isZero()) continue;
    keep.add(a.id);
    let pid = a.parent_account_id;
    while (pid && byId.has(pid) && !keep.has(pid)) {
      keep.add(pid);
      pid = byId.get(pid).parent_account_id;
    }
  }
...
function subtreeTotal(node) {
  let total = new Decimal(node.own);
  for (const child of node.children) total = total.plus(subtreeTotal(child));
  return total;
}
...
/** Emit QuickBooks rows for one account subtree. Returns total as Decimal.
 * detailLevel=summary collapses grandchildren into rolled-up child lines
 * (e.g. Water/Electric/Gas under Rental; Legal Fees / Title under Legal & Professional).
 * detailLevel=detail expands leaves (per-property utilities, each law firm, etc.). */
function emitNode(node, depth, out, drillCtx, detailLevel = 'detail') {
  ...
  if (detailLevel === 'summary') {
    for (const child of children) {
      const childTot = subtreeTotal(child);
      total = total.plus(childTot);
      const nums = collectNumbers(child);
      out.push({
        depth: depth + 1,
        label: child.acc.account_name,
        accountNumber: child.acc.account_number,
        accountId: child.acc.id,
        kind: 'account',
        amount: round2(childTot.toNumber()),
        drill: child.children.length
          ? { kind: 'group', accountNumbers: nums, ...drillCtx }
          : { kind: 'account', accountId: child.acc.id, accountNumber: child.acc.account_number, ...drillCtx },
      });
    }
  } else {
    for (const child of children) {
      total = total.plus(emitNode(child, depth + 1, out, drillCtx, detailLevel));
    }
  }
...
function sectionRows(roots, depth, out, drillCtx, detailLevel = 'detail') {
  let total = new Decimal(0);
  for (const node of roots) total = total.plus(emitNode(node, depth, out, drillCtx, detailLevel));
  return total;
}

/** Net income for Balance Sheet "Net Income" / Current Year Earnings.
 * Calendar YTD only (Jan 1 of asOf year through asOfDate) — must match P&L
 * for the same window. Lifetime P&L is wrong here (Ivymount $215k bug). */
async function computeNetIncome(db, entityId, { asOfDate, startDate, endDate }) {
  if (asOfDate && !startDate) {
    const ytd = await computePeriodNetIncome(db, entityId, calendarYearStart(asOfDate), asOfDate);
    return new Decimal(ytd.netIncome);
  }
  const from = startDate || calendarYearStart(endDate || asOfDate);
  const to = endDate || asOfDate;
  const period = await computePeriodNetIncome(db, entityId, from, to);
  return new Decimal(period.netIncome);
}
...
export async function buildProfitLoss(db, { entityId, startDate, endDate, companyName, detailLevel = 'summary' }) {
  const level = detailLevel === 'detail' ? 'detail' : 'summary';
  ...
  // Keep active leaves plus zero-balance parents so Water/Electric/Gas nest under Rental.
  const active = keepActiveWithAncestors(accounts);
  ...
  const totalRev = sectionRows(buildTree(revA), 2, rows, drillCtx, level);
  ...
  const totalExp = sectionRows(buildTree(expA), 2, rows, drillCtx, level);
