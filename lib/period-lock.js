...
/** First and last calendar day for the month containing dateStr (YYYY-MM-DD or Date). */
export function monthBounds(dateStr) {
  const iso = normalizeIsoDate(dateStr);
  if (!iso) {
    throw new Error(`Invalid posting date for period bounds: ${dateStr}`);
  }
...
export async function assertPeriodOpen(db, entityId, postingDate) {
  const iso = normalizeIsoDate(postingDate) || String(postingDate || '').slice(0, 10);
  ...
  if (closed) {
    const start = normalizeIsoDate(closed.period_start) || closed.period_start;
    const end = normalizeIsoDate(closed.period_end) || closed.period_end;
    throw new Error(`Cannot post to closed period ${start} through ${end}`);
  }
}
