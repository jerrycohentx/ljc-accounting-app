  ...
  for (const acc of FULL_CHART_OF_ACCOUNTS.filter((a) => a.entity === ENTITY_ID)) {
    ...
    if (acc.parent) {
      const parent = await db.get(
        ...
        [ENTITY_ID, acc.parent]
      );
      parentId = parent?.id || null;
    }
    if (parentId) {
      await db.run(
        `UPDATE accounts SET account_name = ?, account_type = ?, normal_balance = ?, parent_account_id = ?
         WHERE entity_id = ? AND account_number = ?`,
        [acc.name, acc.type, normalBalance, parentId, ENTITY_ID, acc.number]
      );
    } else {
      await db.run(
