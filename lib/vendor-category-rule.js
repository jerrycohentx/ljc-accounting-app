...
export async function upsertVendorCategoryRule(db, {
  ...
  const semantic = validateCategorizationCandidate(description || resolved, account);
  if (!semantic.ok) {
    throw new Error(`${semantic.message} Choose a compatible account before saving this vendor rule.`);
  }
...
/**
 * Reclass already-posted journals whose description matches the vendor rule
 * onto the rule's expense account (append-only reclass-offset).
 * Used when Create rule is clicked from reconcile / Fix category so sibling
 * Sam's Club Fuel (etc.) charges update immediately — not only future imports.
 */
export async function applyVendorRuleToPostedJournals(db, {
  entityId,
  pattern,
  accountId,
  matchType = 'contains',
  ...
  const { reclassPostedOffsetLine } = await import('./reclass-posted-offset.js');
  const { resolveEffectiveFromAccount } = await import('./journal-reclass-history.js');

  const needle = normalizePattern(pattern);
  const resolvedMatch = normalizeMatchType(matchType);
  if (!needle || !entityId || !accountId) {
    return { matched: 0, reclassed: 0, skipped: 0, results: [] };
  }
  ...
  if (!account) return { matched: 0, reclassed: 0, skipped: 0, results: [] };

  const bankIds = new Set((bankAccountIds || []).filter(Boolean).map(String));
  const exclude = new Set((excludeJournalIds || []).filter(Boolean).map(String));

  // Broad SQL prefilter, then exact vendorPatternMatches in JS.
  const likeParts = needle.split(/\s+/).filter((p) => p.length >= 2);
  const like = likeParts.length ? `%${likeParts.join('%')}%` : `%${needle}%`;
  ...
  const candidates = await db.all(
    `SELECT id, je_number, description, memo, posting_date
     ...
     WHERE entity_id = ?
       ...
       AND posting_date >= ?
       AND je_number NOT LIKE 'RCLS%'
       AND UPPER(COALESCE(description, '') || ' ' || COALESCE(memo, '')) LIKE ?
     ORDER BY posting_date ASC, je_number ASC
     LIMIT ?`,
    [entityId, startDate, like.toUpperCase(), limit]
  );
  ...
  for (const je of candidates || []) {
    if (exclude.has(String(je.id))) continue;
    const text = [je.description, je.memo].filter(Boolean).join(' ');
    if (!vendorPatternMatches(text, needle, resolvedMatch)) continue;
    matched += 1;

    const lines = await db.all(
      `SELECT jel.id, jel.account_id, jel.debit, jel.credit, jel.line_number,
    ...
    const isBankish = (ln) => {
      if (bankIds.has(String(ln.account_id))) return true;
      const num = String(ln.account_number || '');
      const typ = String(ln.account_type || '').toUpperCase();
      if (/^10\d{2}/.test(num)) return true; // cash / bank
      if (/^20\d{2}/.test(num) && /LIABILITY/.test(typ)) return true; // CC / payables cards
      return false;
    };

    const offsetLines = (lines || []).filter((ln) => {
      if (isBankish(ln)) return false;
      const amt = Math.abs(Number(ln.debit || 0) - Number(ln.credit || 0));
      return amt >= 0.005;
    });
    if (!offsetLines.length) {
      skipped += 1;
      results.push({ jeNumber: je.je_number, skipped: true, reason: 'no_offset_line' });
      continue;
    }

    // Prefer expense/income/cogs; otherwise largest absolute line (same as reconcile category).
    const prefer = offsetLines.filter((l) => /EXPENSE|INCOME|COST OF GOODS|EQUITY/i.test(String(l.account_type || '')));
    ...
    try {
      const effective = await resolveEffectiveFromAccount(db, {
        entityId,
        journalId: je.id,
        lineId: line.id,
        originalAccountId: line.account_id,
        originalAccountNumber: line.account_number,
      });
      if (String(effective.fromAccountId) === String(account.id)) {
        skipped += 1;
        results.push({
      ...
      const out = await reclassPostedOffsetLine(db, {
        journalId: je.id,
        entityId,
        lineId: line.id,
      ...
      results.push({
        jeNumber: je.je_number,
        journalId: je.id,
        fromAccount: effective.fromAccountNumber || line.account_number,
        toAccount: account.account_number,
        amount: Math.abs(Number(line.debit || 0) - Number(line.credit || 0)),
        reclassJeNumber: out.reclassJeNumber,
        alreadyApplied: Boolean(out.alreadyApplied),
      });
    } catch (e) {
      skipped += 1;
      results.push({
        ...
        journalId: je.id,
        skipped: true,
        reason: e.message || 'reclass_failed',
      });
    }
  }
