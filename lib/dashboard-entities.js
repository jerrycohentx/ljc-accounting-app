...
import { suggestCategoryForImport } from './category-learn.js';
...
function inferSourceFromDescription(description) {
  ...
  if (/^Email doc:/i.test(d)) return 'email_doc';
...
export async function getReviewQueue(db, {
  ...
  let items = await Promise.all(rows.map(async (r) => {
    ...
    let suggestedCategoryLabel = null;
    let categoryConfidence = 0;
    let categorySource = null;
    let propertyHint = null;

    if (!r.offset_account_id) {
      try {
        const sug = await suggestCategoryForImport(db, r.entity_id, r.description);
        if (sug.offsetAccountId && !sug.isTransfer && !sug.isChargeback) {
          suggestedCategoryId = sug.offsetAccountId;
          suggestedCategoryLabel = sug.label;
          categoryConfidence = sug.confidence;
          categorySource = sug.categorySource;
        }
        propertyHint = sug.propertyHint || null;
      } catch {
        /* non-fatal */
      }
    } else {
      propertyHint = matchCreVendorHint(r.description);
    }
    ...
    return {
      ...
      categoryConfidence,
      categorySource,
      categorySuggestion: r.rule_label || r.offset_account_name || suggestedCategoryLabel || null,
      propertyHint: propertyHint?.propertyName
        ? `${propertyHint.utilityType ? `${propertyHint.utilityType} · ` : ''}${propertyHint.propertyName}`
        : (propertyHint?.label || null),
      source: src,
      status: 'DRAFT',
