...
import { findMatchingBankPayment } from './amex-import-commit.js';
...
export async function commitBankImportTransactions(db, {
  ...
  const isCardLiability = isLiabilityAccount(bankAccount);
  const defaultExpense = isCardLiability
    ? await resolveAccountByNumber(db, entityId, '5700')
    : null;
  let createdJECount = 0;
  let skippedCardPayments = 0;
  const importedTransactions = [];

  for (const txn of newTransactions) {
    // Credit-card feed on a liability account: never invent bank cash.
    // Payments match/skip; charges & merchant credits use expense offsets only.
    if (isCardLiability && isCardPaymentTxn(txn) && !isMerchantCreditTxn(txn)) {
      const bankMatch = String(bankAccount.account_number) === '2010'
        ? await findMatchingBankPayment(db, entityId, { amount: txn.amount, date: txn.date })
        : null;
      skippedCardPayments += 1;
      importedTransactions.push({
        fitid: txn.fitid,
        status: bankMatch ? 'MATCHED_BANK' : 'UNMATCHED_PAYMENT',
        bankJeId: bankMatch?.je_id || null,
        amount: txn.amount,
    ...
    let offsetAccountId = cat.offsetAccountId || defaultOffsetId;
    if (isCardLiability) {
      // Card feeds must never offset to cash / undeposited funds.
      try {
        await assertNotCashOffset(db, entityId, offsetAccountId, 'Card liability import');
      } catch {
        if (!defaultExpense?.id) throw new Error('Expense account 5700 required for card imports');
        offsetAccountId = defaultExpense.id;
      }
      if (offsetAccountId === defaultOffsetId) {
        if (!defaultExpense?.id) throw new Error('Expense account 5700 required for card imports');
        offsetAccountId = defaultExpense.id;
      }
    }
    ...
    // Asset bank: deposit = DR cash. Card liability: charge = CR card; merchant credit = DR card.
    const cardMerchantCredit = isCardLiability && (isMerchantCreditTxn(txn) || txn.isCredit === true);
    const isDeposit = isCardLiability ? cardMerchantCredit : !!txn.isCredit;
...
/**
 * Every source transaction id (fitid) this entity has EVER downloaded, across
 * every feed and every status -- DRAFT, MATCHED, POSTED, and REJECTED.
 *
 * REJECTED is intentionally included: a transaction the user discarded was
 * still downloaded once, and "only download new, never before downloaded"
 * means it must never be pulled in again. The REJECTED row is a permanent
 * tombstone for its fitid. Any download path that pre-filters on this set
 * (Plaid, Ramp, Amex, OFX, statement catch-ups) therefore skips anything
 * already downloaded.
 */
 ...
 */
export function deriveRulePattern(description) {
