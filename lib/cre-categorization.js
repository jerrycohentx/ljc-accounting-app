...
/** Ensure Water / Electric / Gas (and catch-all Utilities) nest under Rental Property Expenses (6100). */
export async function ensureRentalUtilityTypeParents(db, entityId) {
  ...
  // Legacy catch-all 5500 Utilities → under Rental Property Expenses
  const utilCatchAll = await db.get(
    `SELECT id, parent_account_id FROM accounts WHERE entity_id = ? AND account_number = '5500' LIMIT 1`,
    [entityId]
  );
  if (utilCatchAll?.id && utilCatchAll.parent_account_id !== rentalId) {
    await db.run(
      `UPDATE accounts SET parent_account_id = ? WHERE id = ?`,
      [rentalId, utilCatchAll.id]
    );
  }
...
export async function seedCreCategorizationRules(db, entityId = 'ent-ljc') {
  ...
  for (const vp of config.vendorPatterns || []) {
    ...
    const typeFallback = RENTAL_UTILITY_TYPE_PARENTS[utility]?.number || null;
    const offset = prop?.utilityAccounts?.[utility] || typeFallback;
    if (!offset) continue;
