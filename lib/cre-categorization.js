...
export async function ensurePropertyUtilityAccounts(db, entityId, config = loadCreCategorizationRules()) {
  ...
  for (const prop of config.properties || []) {
    ...
    const typeParentId = reimbursable ? (arParent?.id || null) : parentId;
    ...
    for (const [utility, number] of Object.entries(prop.utilityAccounts || {})) {
      ...
      if (!row) {
        ...
        created += 1;
      } else if (reimbursable && row.account_type !== 'ASSET') {
        await db.run(
          `UPDATE accounts
           SET account_name = ?, account_type = 'ASSET', normal_balance = 'DEBIT',
               parent_account_id = COALESCE(?, parent_account_id), updated_at = CURRENT_TIMESTAMP
           WHERE id = ?`,
          [name, typeParentId, row.id]
        );
      }
