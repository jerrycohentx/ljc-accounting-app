...
async function allocateOpeningJeNumber(db, entityId, asOfDate) {
  const base = `OB-${asOfDate.replace(/-/g, '')}`;
  ...
  if (active) {
    throw new Error(`Opening balance ${base} already posted for this entity; reverse it first or use --force refresh`);
  }
  const used = await db.get(
    `SELECT je_number FROM journal_entries WHERE entity_id = ? AND je_number LIKE ? ORDER BY created_at DESC LIMIT 1`,
    [entityId, `${base}%`]
  );
  if (!used) return base;
  if (used.je_number === base) return `${base}-2`;
  const m = used.je_number.match(new RegExp(`^${base.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}-(\\d+)$`));
  const seq = m ? parseInt(m[1], 10) + 1 : 2;
  return `${base}-${seq}`;
}
...
export async function postOpeningBalances(db, { entityId, asOfDate, balances, userId, memo }) {
  ...
  const jeNumber = await allocateOpeningJeNumber(db, entityId, asOfDate);
