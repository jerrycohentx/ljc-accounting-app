...
export async function postOpeningBalances(db, { entityId, asOfDate, balances, userId, memo }) {
  ...
  const existing = await db.get(
    `SELECT id FROM journal_entries WHERE entity_id = ? AND je_number = ? AND status = 'POSTED'`,
    [entityId, jeNumber]
  );
  if (existing) {
    throw new Error(`Opening balance ${jeNumber} already posted for this entity; reverse it first`);
  }
