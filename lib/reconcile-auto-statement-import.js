...
async function postReconcileImportDrafts(db, { entityId, accountId, userId, importId = null }) {
  let posted = 0;
  const postErrors = [];

  const postOne = async (journalEntryId) => {
    try {
      ...
      postErrors.push({ journalId: journalEntryId, error: err.message });
      console.warn('reconcile auto-post JE failed:', journalEntryId, err.message);
    }
  };
  ...
  if (importId) {
    ...
    for (const row of rows) {
      await postOne(row.journal_entry_id);
    }
  }

  // Prior partial runs can leave DRAFT reconcile imports that dedup blocks on retry.
  const pending = await db.all(
    `SELECT DISTINCT je.id AS journal_entry_id
     FROM journal_entries je
     JOIN journal_entry_lines jel ON jel.journal_entry_id = je.id AND jel.account_id = ?
     WHERE je.entity_id = ? AND je.status = 'DRAFT'
       AND je.description LIKE 'Reconcile:%'
       AND je.reversed_by_je_id IS NULL AND je.reverses_je_id IS NULL
     ORDER BY je.posting_date ASC, je.je_number ASC`,
    [accountId, entityId]
  );
  ...
  return { posted, postErrors: postErrors.length ? postErrors : undefined };
}
...
async function runEnsureStatementRegisterFromPdf(db, opts) {
  ...
  if (!force && isCard && postedCount >= Math.max(1, txnCount - 2)) {
    const { posted, postErrors } = await postReconcileImportDrafts(db, {
      entityId,
      accountId,
      userId,
    });
  ...
  return {
    ...
    duplicateDetail: importResult.duplicateDetail,
    matchedPayments: importResult.matchedPayments,
    unmatchedPayments: importResult.unmatchedPayments,
    statementLines: txnCount,
