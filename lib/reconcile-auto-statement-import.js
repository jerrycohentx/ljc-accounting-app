...
async function parseStatementPdfBuffer(buf, { entityId, accountNumber }) {
  ...
  if (last4 && looksLikeChaseCcStatement(text, last4)) {
    return extractChaseCcPdfFromBuffer(buf, { last4 });
  }
  const tmp = `${process.cwd()}/_tmp_recon_parse_${Date.now()}.pdf`;
 ...
 */
export async function ensureStatementRegisterFromPdf(db, {
  entityId,
  accountId,
  accountNumber,
  statementDate,
  ...
  try {
    ...
    return { skipped: true, reason: 'resolve-failed', error: err.message };
  }

  if (!statementFile?.file_data) {
    return { skipped: true, reason: 'no-statement-pdf' };
  }
  ...
  const importResult = await importStatementForReconcile(db, {
    ...
    userId,
    pdfBase64,
    fileName: statementFile.file_name || 'statement.pdf',
    autoPost: false,
    createEntries: true,
  });
