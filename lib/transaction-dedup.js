...
/**
 * Every previously-imported row for this entity+account, any source, ANY
 * status (including REJECTED). The `status` column is carried so callers can
 * treat an exact-fitid match as "already downloaded" regardless of status
 * (a discarded transaction must never be re-downloaded), while restricting
 * the fuzzy content match to non-rejected rows.
 */
const KNOWN_ROWS_SQL = `
  SELECT it.date, it.amount, it.description, it.fitid, it.status
...
export async function filterNewTransactions(db, entityId, accountId, transactions) {
  ...
  // Exact fitid = the same source transaction. Block it if it was EVER
  // downloaded, including if it was later discarded (REJECTED) -- once
  // downloaded, never downloaded again.
  ...
  // Fuzzy content match (date + amount + similar description) stays scoped to
  // NON-rejected rows only: a genuinely new transaction that merely resembles
  // one the user previously discarded should still surface for review.
  const buckets = buildContentBuckets(existing.filter((r) => r.status !== 'REJECTED'));
