...
/** Stable ISO-ish key for sorting (handles Date objects and PG/SQLite strings). */
...
export async function getReclassHistoryForJournal(db, entityId, journalId, lines = []) {
  ...
  items.sort((a, b) => {
    const da = sortTimeKey(a.postingDate);
    const dbDate = sortTimeKey(b.postingDate);
    if (da !== dbDate) return da.localeCompare(dbDate);
    const ca = sortTimeKey(a.createdAt);
    const cb = sortTimeKey(b.createdAt);
    if (ca !== cb) return ca.localeCompare(cb);
    // Manual Fix category must win over an earlier rules reclass when times tie.
    const rank = (s) => (s === 'offset' ? 1 : 0);
    const src = rank(a.source) - rank(b.source);
    if (src) return src;
