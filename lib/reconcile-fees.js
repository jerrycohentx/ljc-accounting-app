...
export async function postReconcileFees(db, {
  ...
  if (svc > 0.005) {
    ...
    const jeNumber = `SVC-CHG-${accountNumber}-${postingDate.replace(/-/g, '')}`;
  ...
  if (intr > 0.005) {
    const incomeId = await findAccountId(db, entityId, {
      type: 'REVENUE',
    ...
    const jeNumber = `INT-ERND-${accountNumber}-${postingDate.replace(/-/g, '')}`;
    const { jeId, accountGlId } = await ensureFeeJournal(db, {
      entityId,
      jeNumber,
      postingDate,
