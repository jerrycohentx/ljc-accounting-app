...
export async function applyWentworthTenantUtilityTreatment(db, {
  ...
  // Verizon cellphone is Telephone Expense (5760), not Computer & Internet / Wentworth.
  await db.run(
    `UPDATE bank_categorization_rules
     SET offset_account_number = '5760', label = 'Telephone — Verizon', priority = 10
     WHERE entity_id = ? AND pattern IN ('VERIZONWRLSS', 'VERIZON', 'VZWIRELESS')`,
    [entityId]
  );
