/**
 * Wentworth (3427) utilities are tenant-reimbursed — book as receivable assets,
 * not P&L expenses (gas / electric / water / internet).
 */
...
const WENTWORTH_ACCOUNTS = [
  {
    ...
    description: 'Tenant-reimbursable gas (CenterPoint ENTEX). Not an expense.',
  },
  {
    number: '6252',
    name: 'Due from Tenant — Electric — 3427 Wentworth',
    description: 'Tenant-reimbursable electric (Reliant / Just Energy). Not an expense.',
  },
  {
    number: '6253',
    name: 'Due from Tenant — Water — 3427 Wentworth',
    description: 'Tenant-reimbursable water. Not an expense.',
  },
  {
    number: '6254',
    name: 'Due from Tenant — Internet — 3427 Wentworth',
    description: 'Tenant-reimbursable internet (Comcast / Xfinity). Not an expense.',
  },
];

const RULE_UPDATES = [
  { pattern: 'CPENERGY ENTEX', offset: '6251', label: 'Wentworth gas — tenant reimbursable', priority: 12 },
  { pattern: 'ENTEX CNP', offset: '6251', label: 'Wentworth gas — tenant reimbursable', priority: 12 },
  { pattern: 'CENTERPOINT ENERGY', offset: '6251', label: 'Wentworth gas — tenant reimbursable', priority: 13 },
  ...
  { pattern: 'PENNYWISE POWER', offset: '6252', label: 'Wentworth electric — tenant reimbursable', priority: 14 },
...
export async function applyWentworthTenantUtilityTreatment(db, {
  ...
  return {
    ...
    utilBalances,
    message: dryRun
      ? `Accounts flipped to tenant receivable; dry run would move ${internetReclass.reclassed} Comcast line(s)`
      : 'Wentworth utilities are tenant-reimbursable assets; Comcast moved off expense',
  };
}
