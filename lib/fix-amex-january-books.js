...
export async function fixAmexJanuaryBooks(db, {
  ...
  // 2010 is 88007 only. Any residual above the 88007 statement belongs on 2012
  // (other Amex cards) — same split as CLEAN-20260101, not a 3900/2999 plug.
  const gap = report.gapAfterCleanup;
  if (Math.abs(gap) >= 0.005) {
    const a2010 = await db.get(
      `SELECT id FROM accounts WHERE entity_id = ? AND account_number = '2010'`,
      [entityId]
    );
    const a2012 = await db.get(
      `SELECT id FROM accounts WHERE entity_id = ? AND account_number = '2012'`,
      [entityId]
    );
    if (!a2012) {
      report.reclassError = 'Account 2012 not found — cannot reclass residual off 88007';
    } else if (dryRun) {
      report.steps.reclassTo2012 = {
        dryRun: true,
        amount: gap,
        description:
          'Reclass non-88007 residual from 2010 to 2012 so 2010 equals Amex 88007 stmt 2026-01-09',
      };
      report.afterCleanupBalance = JAN_TARGET;
      report.gapAfterCleanup = 0;
    } else {
      const jeId = `je-${uuidv4()}`;
      const amount = Math.abs(gap);
      // gap > 0 means 2010 liability too high → debit 2010, credit 2012
      const lines =
        gap > 0
          ? [
              {
                ...
                debit: amount,
                credit: 0,
                description: 'Move residual off 88007 to other Amex cards',
              },
              {
                accountId: a2012.id,
                debit: 0,
                credit: amount,
                description: 'Receive residual from 88007 split',
              },
            ]
          : [
              {
                ...
                debit: amount,
                credit: 0,
                description: 'Return residual to 88007 from other Amex cards',
              },
              {
                accountId: a2010.id,
                debit: 0,
                credit: amount,
                description: 'Restore 88007 to statement ending',
              },
            ];
      const jeNumber = `AMEX-88007-SPLIT-${JAN_STMT.replace(/-/g, '')}`;
      const desc =
        'Reclass non-88007 Amex residual from card 88007 (2010) to other Amex cards (2012) to match 88007 statement ending';
      await db.run(
        `INSERT INTO journal_entries
         (id, entity_id, je_number, posting_date, description, memo, status, created_by, total_debit, total_credit, source)
         VALUES (?, ?, ?, ?, ?, ?, 'DRAFT', ?, ?, ?, ?)`,
        [
          jeId,
          entityId,
      ...
      for (const line of lines) {
        await db.run(
          `INSERT INTO journal_entry_lines
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [`jel-${uuidv4()}`, jeId, line.accountId, line.debit, line.credit, line.description, ln++]
        );
      }
      await postJournalEntryToGl(db, { journalId: jeId, entityId, userId });
      report.steps.reclassTo2012 = { jeId, amount: gap, posted: true };
      const afterReclass = await getPostedBankBalance(db, entityId, CARD, JAN_STMT);
      report.afterCleanupBalance = afterReclass?.balance ?? null;
      report.gapAfterCleanup = round2((afterReclass?.balance ?? 0) - JAN_TARGET);
    }
  }
  ...
  if (!dryRun && rebuildRecon) {
    ...
    for (const t of targets) {
      const bookAtStmt = await getPostedBankBalance(db, entityId, CARD, t.statementDate);
      const bookBal = round2(bookAtStmt?.balance ?? 0);
      // When books already equal the statement, close with beginning = ending
      // (activity is already in the register — do not pull post-statement lines).
      const beginOverride =
        Math.abs(bookBal - round2(t.endingBalance)) < 0.005 ? round2(t.endingBalance) : null;
      const r = await autoReconcileToTarget(db, {
        ...
        clearedAfterDate: '2025-12-09',
        beginningBalanceOverride: beginOverride,
      });
      report.reconResults.push({ ...r, bookAtStatement: bookBal, beginOverride });
    }
  }
