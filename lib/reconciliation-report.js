...
export async function buildReconciliationReport(db, {
  ...
  // For a CLOSED recon that ties to the bank, ending = statement ending.
  // Do not roll opening-balance leftovers into a second "ending" that disagrees
  // with the statement (that made recons look unreconciled).
  const sessionTied =
    session?.status === 'CLOSED' &&
    statementEndingBalance != null &&
    toCents(clearedBalance) === toCents(statementEndingBalance);
  const endingBalance = sessionTied ? statementEndingBalance : registerBalance;
