/**
 * Extract bank / credit-card statement PDF → JSON (Node/pdf-parse — works on Render without Python).
 */

import fs from 'fs';
import path from 'path';
import crypto from 'crypto';
import { extractAmexPdfFromFile, looksLikeAmexStatement } from './extract-amex-pdf.js';
...
function parseMetadata(text) {
  ...
  // Account identification — feeds the cross-account upload guard, so failure
  // here is a SECURITY gap, not a cosmetic one. The old pattern required exactly
  // one space ("Account Number X+") while real statements pad with many
  // ("Account Number          XXXXXXXXXXXX0260"), so accountLast4 was null on
  // every Simmons statement and the guard silently never fired (2026-07-16: a
  // Simmons PDF sailed into Lone Star twice). \s+ tolerates any spacing, and
  // "Primary Account" is the header variant on page 1.
  m = text.match(/(?:Account\s+Number|Primary\s+Account)\s+X+(\d{4})/i);
  if (m) meta.accountLast4 = m[1];
  m = text.match(/Account\s+Number\s+Ending\s+(\d{4})/i);
  if (m && !meta.accountLast4) meta.accountLast4 = m[1];
  if (meta.accountLast4 === '7367') meta.bankName = 'Lone Star Bank';
  else if (meta.accountLast4 === '0260' || meta.accountLast4 === '7036') {
...
export async function extractPdfStatementFromFile(pdfPath) {
  ...
  // Amex credit-card statements (reconcile upload was failing with "not a supported checking statement").
  if (looksLikeAmexStatement(text)) {
    return extractAmexPdfFromFile(pdfPath);
  }

  if (looksLikeChaseCcStatement(text)) {
    const last4 = (pdfPath.match(/(\d{4})/) || [])[1] || null;
    return extractChaseCcPdfFromBuffer(buf, { last4: last4 || '6508' });
  }

  if (!text.includes('CHECKING ACCOUNTS') && !text.includes('Current Balance') && !text.includes('Ending Balance')) {
    throw new Error('not a supported checking or Amex credit-card statement');
  }
  const meta = parseMetadata(text);
