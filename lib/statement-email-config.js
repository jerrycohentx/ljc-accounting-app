...
import { isRebuildFreezeActive } from './rebuild-freeze.js';
...
export function getEmailIngestSettings() {
  // Master gate is REBUILD_FREEZE. Legacy STATEMENT_EMAIL_ENABLED=0 on Render
  // from the July rebuild must not keep email permanently off.
  const legacyOff = process.env.STATEMENT_EMAIL_ENABLED === '0';
  const forceOff = ['force-off', 'never'].includes(String(process.env.STATEMENT_EMAIL_ENABLED || '').toLowerCase());
  const enabled = !isRebuildFreezeActive() && !forceOff && !(legacyOff && process.env.HONOR_LEGACY_EMAIL_OFF === '1');
  return {
    enabled,
    ...
    searchQuery: process.env.STATEMENT_EMAIL_SEARCH_QUERY
      || '(has:attachment (statement OR bank OR simmons OR "lone star" OR amex OR shellpoint)) OR (from:lsbtexas.com OR from:info@lsbtexas.com OR subject:estatement)',
