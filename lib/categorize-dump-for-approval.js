...
export async function categorizeDumpForApproval(db, {
     ...
     WHERE gl.entity_id = ?
       ...
       AND NOT EXISTS (
         ...
         WHERE d.entity_id = je.entity_id
           AND d.status IN ('DRAFT', 'POSTED', 'PENDING_APPROVAL', 'APPROVED')
           AND d.reversed_by_je_id IS NULL
           AND (
             d.memo LIKE ('reclass-rules:' || je.id || ':' || gl.id || '%')
             -- One CAT-APPR per source journal (gl id may change after rebuilds)
             OR d.memo LIKE ('cat-approve:' || je.id || '%')
           )
       )
  ...
  let draftsCreated = 0;
  let docsCount = 0;
  // In-run guard against concurrent/loop races creating a second CAT-APPR
  // for the same source JE (or same gl line).
  const createdSourceKeys = new Set();
  ...
  for (const row of rows) {
    ...
    const sourceKey = String(row.journal_id);
    const glKey = `${row.journal_id}:${row.gl_id}`;
    if (createdSourceKeys.has(sourceKey) || createdSourceKeys.has(glKey)) {
      continue;
    }
    ...
    // Pre-insert re-check (closes TOCTOU gap if two runs race).
    const already = await db.get(
      `SELECT id, je_number, status FROM journal_entries
       WHERE entity_id = ?
         AND status IN ('DRAFT', 'POSTED', 'PENDING_APPROVAL', 'APPROVED')
         AND reversed_by_je_id IS NULL
         AND memo LIKE ?
       LIMIT 1`,
      [entityId, `cat-approve:${row.journal_id}%`]
    );
    if (already) {
      createdSourceKeys.add(sourceKey);
      createdSourceKeys.add(glKey);
      proposed.push({
        ...entry,
        skippedDuplicate: true,
        existingDraftId: already.id,
        existingJeNumber: already.je_number,
      });
      continue;
    }
