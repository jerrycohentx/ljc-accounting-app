 ...
 * Exact date first; then nearest stored file within fuzzyDays (Simmons cycles
 * often end on the 1st while the PDF was saved under periodEnd).
 */
export async function getStatementFile(db, { entityId, accountId, statementDate, fuzzyDays = 5 }) {
  ...
  for (const row of rows || []) {
    const sd = String(row.statement_date || '').slice(0, 10);
