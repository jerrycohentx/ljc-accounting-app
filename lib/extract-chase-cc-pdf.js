...
export async function extractChaseCcPdfFromBuffer(buf, { last4 = '6508' } = {}) {
  const text = await parsePdfBuffer(buf);
  if (!looksLikeChaseCcStatement(text, last4)) {
    throw new Error('not a Chase credit card statement');
  }
  const meta = parseChaseCcMetadata(text, { last4 });
  const transactions = parseChaseCcTransactions(text, meta);
  return {
