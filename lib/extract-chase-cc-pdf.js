...
export function parseChaseCcTransactions(text, meta = {}) {
  ...
  while ((m = lineRe.exec(slice)) !== null) {
    ...
    const baseFitid = fitid(date, amount, desc);
    const seen = fitidCounts.get(baseFitid) || 0;
    fitidCounts.set(baseFitid, seen + 1);
    const txnFitid = seen ? `${baseFitid}-${seen}` : baseFitid;
    txns.push({
      date,
