...
export function computeExpectedDepositDate(managementCompany, periodEnd) {
  ...
  // Postgres returns DATE columns as full ISO timestamps ("2026-06-30T00:00:00.000Z");
  // SQLite returns the plain "2026-06-30" string. Accept either — just need the date part.
  const m = String(periodEnd).slice(0, 10).match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (!m) return null;
  let year = Number(m[1]);
