...
export async function isPasswordResetEmailAvailable(db = null) {
  const channels = getPasswordResetEmailChannels(db);
  if (channels.resend || channels.smtp || channels.graph || channels.statementEnv || channels.gmailOAuthEnv) {
    return true;
  }
  if (!db) return false;
  const mailboxes = await resolveAllMailboxes(db);
  return mailboxes.some((m) => m.password || m.refresh_token);
}
