...
/** Send a general app email (digests, notifications) using the same channel fallbacks. */
export async function sendAppEmail({ to, subject, text, html, db = null }) {
  const attempts = [];
  ...
  for (const acct of getStatementEmailAccounts()) {
    ...
    attempts.push(async () => {
      ...
      return { transport: 'smtp', from: acct.user, to };
    });
  }

  const oauthAccounts = [...getGmailOAuthAccounts()];
  if (db) {
    const mailboxes = await resolveAllMailboxes(db);
    for (const mb of mailboxes) {
      if (mb.refresh_token) oauthAccounts.push({ user: mb.user, refresh_token: mb.refresh_token });
    }
  }

  if (isGmailOAuthConfigured() && oauthAccounts.length) {
    attempts.push(async () => {
      const errors = [];
      for (const acct of oauthAccounts) {
        if (!acct.refresh_token) continue;
        try {
          return await sendGmailOAuthMail({
            refreshToken: acct.refresh_token,
            from: acct.user,
            to,
            subject,
      ...
      throw new Error(errors.join('; ') || 'No Gmail OAuth accounts available');
    });
  }
  ...
  throw new Error(
    errors.length
      ? `All email channels failed: ${errors.slice(0, 3).join(' | ')}`
      : 'Email not configured for outbound digests'
  );
}
