  ...
  const rows = await db.all(
    `SELECT orig.id AS orig_id, orig.je_number AS orig_number, orig.posting_date,
     ...
     WHERE orig.entity_id = ?
       ...
       AND (orig.je_number LIKE 'IMP-%' OR orig.je_number LIKE 'AMEX-%')
       AND orig.posting_date >= '2026-01-01' AND orig.posting_date <= '2026-06-30'
       AND (
         COALESCE(rev.memo, '') LIKE 'Reverse duplicate of IMP-%'
         OR COALESCE(rev.memo, '') LIKE 'Reverse duplicate of AMEX-%'
         OR COALESCE(rev.description, '') LIKE 'Reverse duplicate of IMP-%'
         OR COALESCE(rev.description, '') LIKE 'Reverse duplicate of AMEX-%'
       )
       AND NOT EXISTS (
