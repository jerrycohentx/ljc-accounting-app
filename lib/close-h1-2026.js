...
import { reverseDuplicateBankImports, restoreMistakenImportReversals } from './reverse-duplicate-bank-imports.js';
...
export async function closeH1_2026(
  ...
  // Restore any IMP/AMEX lines that an earlier aggressive dedupe reversed by mistake
  report.restoreImports = await restoreMistakenImportReversals(db, { entityId: ENTITY_ID, userId });

  // Undo duplicate manual JE twins only (never IMP vs IMP)
