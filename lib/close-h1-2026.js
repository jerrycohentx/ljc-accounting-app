...
import { clearOrphanReconciledForH1Banks } from './clear-orphan-reconciled.js';
import { reverseLonestarStatementMismatches } from './reverse-lonestar-statement-mismatches.js';
...
export async function closeH1_2026(
  ...
  // Remove Lonestar postings that are not on Lonestar statements (Mercury + Simmons twin)
  report.lonestarMismatches = await reverseLonestarStatementMismatches(db, {
    entityId: ENTITY_ID,
    userId,
  });

  // Unstick Apr–Jun lines marked RECONCILED without a valid covering CLOSED session
  report.orphanClears = await clearOrphanReconciledForH1Banks(db, { entityId: ENTITY_ID });
