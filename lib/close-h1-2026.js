...
export async function closeH1_2026(
  ...
  // Undo RESTORE-* journals from a prior bad restore pass (they re-introduced true dups)
  report.undoRestores = await reverseRestoreImportJournals(db, { entityId: ENTITY_ID, userId });

  // Only reverse manual JE-* twins when IMP/AMEX feed line exists (safe dedupe)
  report.dedupe = await reverseDuplicateBankImports(db, { entityId: ENTITY_ID, userId });

  // Do NOT auto-restore IMP/AMEX reversals here — that reintroduced true duplicates.
  report.restoreImports = { skipped: true, reason: 'disabled — use manual review if needed' };
