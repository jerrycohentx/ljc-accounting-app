...
// Postgres hands back DATE/TIMESTAMP columns as JS Date objects; ISO-string
// dates should normalize to YYYY-MM-DD. Any other pre-formatted label (some
// older archived reports stored line dates as "Fri Jan 02") is shown verbatim
// -- never fed to new Date(), which would invent a bogus year (e.g. 2001).
function fmtDate(d) {
  if (!d) return '';
...
export async function renderReconciliationReportPdf(report, { mode = 'both' } = {}) {
  ...
  // Flags for the memory-constrained Render free instance (512MB shared with
  // Node + Postgres): single-process + no-zygote keep Chromium's footprint
  // small so laying out a large multi-page report doesn't thrash into an OOM
  // or a setContent stall. /dev/shm is tiny (--disable-dev-shm-usage), and
  // sandboxing/GPU aren't available in the container.
