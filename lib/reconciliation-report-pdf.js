...
function summaryHtml(report) {
  ...
  const variance = s.statementVariance;
  ...
  if (hasUncleared) {
    rows.push('<div class="sum-group">Uncleared Transactions</div>');
    ...
    rows.push(sumRow('Register Balance as of statement date', s.registerBalance, { total: true }));
  }

  const hasNew = (nw.paymentsCount || 0) + (nw.depositsCount || 0) > 0;
  if (hasNew) {
    rows.push('<div class="sum-group">New Transactions (after statement date)</div>');
    rows.push(sumRow(`${paymentsLabel} (${nw.paymentsCount || 0})`, nw.paymentsTotal, { indent: true }));
    rows.push(sumRow(`${depositsLabel} (${nw.depositsCount || 0})`, nw.depositsTotal, { indent: true }));
  }

  rows.push(sumRow('Ending Balance (register)', s.endingBalance, { total: true }));
  if (s.statementEndingBalance != null) {
    rows.push(sumRow('Statement Ending Balance', s.statementEndingBalance));
    rows.push(sumRow('Difference', variance == null ? 0 : variance, { total: true }));
  }
...
function detailHtml(report) {
  ...
  if (hasUncleared) {
    parts.push(detailTable(`Uncleared ${paymentsLabel}`, uncleared.payments, uncleared.total != null ? uncleared.total : null));
    parts.push(detailTable(`Uncleared ${depositsLabel}`, uncleared.deposits, null, uncleared.balance));
  }

  const hasNew = (nw.payments || []).length + (nw.deposits || []).length > 0;
  if (hasNew) {
    parts.push(detailTable(`New ${paymentsLabel}`, nw.payments, nw.total != null ? nw.total : null));
    parts.push(detailTable(`New ${depositsLabel}`, nw.deposits, null, nw.balance));
  }

  parts.push(sumRow('Ending Balance (register)', d.endingBalance, { total: true }));
...
export function buildReportHtml(report, { mode = 'both' } = {}) {
  ...
  const generated = report.generated_at ? String(report.generated_at).replace('T', ' ').slice(0, 19) : '';
