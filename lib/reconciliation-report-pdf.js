 ...
 * → Register Balance → Ending Balance roll-forward (as of the statement date
 * only — never "New Transactions" dated after the statement).
 */
...
function summaryHtml(report) {
  ...
  // Never show "New Transactions" (post-statement-date activity) on a recon report.
...
function detailHtml(report) {
  ...
  const asOf = s.registerBalanceAsOf || report.statement_date;
  const statementIso = String(report.statement_date || asOf || '').slice(0, 10);

  function onOrBeforeStatement(items) {
    return (items || []).filter((r) => {
      const d0 = String(r.date || '').slice(0, 10);
      return !statementIso || !d0 || d0 <= statementIso;
    });
  }
  ...
  body.push(detailGroup(pl, onOrBeforeStatement(cl.payments), true));
  body.push(detailGroup(dl, onOrBeforeStatement(cl.deposits), true));
  ...
  // Uncleared Transactions (through statement date only)
  const unPay = onOrBeforeStatement(un.payments);
  const unDep = onOrBeforeStatement(un.deposits);
  const hasUncleared = unPay.length + unDep.length > 0;
  if (hasUncleared) {
    body.push('<tr class="grp"><td colspan="7">Uncleared Transactions</td></tr>');
    body.push(detailGroup(pl, unPay, false));
    body.push(detailGroup(dl, unDep, false));
  ...
  // Never include post-statement-date "New Transactions".
