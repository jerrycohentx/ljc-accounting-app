...
import { ensureStatementRegisterFromPdf } from './reconcile-auto-statement-import.js';
import { sqlExcludeNonBankReconJournals, tagYeReclassJournalSources } from './bank-recon-exclude.js';
...
function sessionDisplay(session, clearedCount, live = null) {
  ...
  return {
    statementDate: normalizeIsoDate(session.statement_date) || session.statement_date,
...
export async function ensureBankReconSessionTables(db) {
  ...
  await tagYeReclassJournalSources(db);
}
...
export async function getBeginningBalance(db, entityId, accountId, statementDate, normalBalance) {
  ...
  const bundled = account?.account_number
    ? peekBundledStatement(account.account_number, statementDate, entityId)
    : null;
...
/**
 * Earliest OPEN reconciliation for one account (resume target).
 * Optional year (e.g. 2026) limits to that calendar year's statement dates.
 */
export async function getEarliestOpenSession(db, entityId, accountId, { year = null } = {}) {
  await ensureBankReconSessionTables(db);
  if (year != null && Number.isFinite(Number(year))) {
    ...
    return db.get(
      `SELECT * FROM bank_reconciliation_sessions
       ...
       ORDER BY statement_date ASC LIMIT 1`,
      [entityId, accountId, `${y}-01-01`, `${Number(y) + 1}-01-01`]
    );
  }
  return db.get(
    `SELECT * FROM bank_reconciliation_sessions
...
/**
 * Earliest OPEN reconciliation across all accounts for an entity.
 * Used when Jerry opens the Reconcile tab with no deep link — resume that
 * worksheet instead of Begin Reconciliation with a mismatched statement.
 */
export async function getEarliestOpenSessionForEntity(db, entityId, { year = null } = {}) {
  await ensureBankReconSessionTables(db);
  const params = [entityId];
  let yearClause = '';
  if (year != null && Number.isFinite(Number(year))) {
    const y = String(Number(year));
    yearClause = ' AND s.statement_date >= ? AND s.statement_date < ?';
    params.push(`${y}-01-01`, `${Number(y) + 1}-01-01`);
  }
  return db.get(
    `SELECT s.*, a.account_number, a.account_name
     FROM bank_reconciliation_sessions s
     JOIN accounts a ON a.id = s.account_id AND a.entity_id = s.entity_id
     WHERE s.entity_id = ? AND s.status = 'OPEN'${yearClause}
     ORDER BY s.statement_date ASC, a.account_number ASC
     LIMIT 1`,
    params
  );
}
...
/**
 * Attach effective offset/category for each register line so the reconcile UI
 * can show Category without drilling into every JE.
 *
 * Resolution order for the non-bank side of the JE:
 * 1) Latest unreverted reclass-offset:<jeId>:<lineId>:<toAccountId>
 * 2) Latest unreverted reclass-rules:<jeId>:<dumpGlId> (+ chained reclass-offset on that RCLS JE)
 * 3) Largest non-bank JE line (expense/income preferred; "Split" when ≥2 material offsets)
 */
async function enrichEntriesWithCategory(db, entityId, bankAccountId, entries) {
  if (!entries?.length) return entries || [];
  const jeIds = [...new Set(entries.map((e) => e.journal_entry_id).filter(Boolean))];
  if (!jeIds.length) return entries;

  const ph = jeIds.map(() => '?').join(',');
  const offsetLines = await db.all(
    `SELECT jel.id AS line_id, jel.journal_entry_id, jel.account_id,
            ...
            (
              ...
              WHERE gl.journal_entry_id = jel.journal_entry_id
                AND gl.account_id = jel.account_id
                AND gl.entity_id = ?
                AND ABS(COALESCE(gl.debit, 0) - COALESCE(jel.debit, 0)) < 0.02
                AND ABS(COALESCE(gl.credit, 0) - COALESCE(jel.credit, 0)) < 0.02
              ORDER BY gl.id
              LIMIT 1
            ) AS gl_id
     FROM journal_entry_lines jel
     JOIN accounts a ON a.id = jel.account_id
     WHERE jel.journal_entry_id IN (${ph})
       AND jel.account_id <> ?
     ORDER BY jel.journal_entry_id, jel.line_number`,
    [entityId, ...jeIds, bankAccountId]
  );

  const byJe = new Map();
  for (const row of offsetLines || []) {
    const list = byJe.get(row.journal_entry_id) || [];
    list.push(row);
    byJe.set(row.journal_entry_id, list);
  }

  // Latest reclass-offset per (jeId, lineId) → toAccountId
  const offsetTarget = new Map(); // `${jeId}:${lineId}` → { id, number, name }
  const offsetRows = await db.all(
    `SELECT memo FROM journal_entries
     WHERE entity_id = ? AND status = 'POSTED' AND reversed_by_je_id IS NULL
       AND (${jeIds.map(() => 'memo LIKE ?').join(' OR ')})
     ORDER BY posting_date ASC, created_at ASC, je_number ASC`,
    [entityId, ...jeIds.map((id) => `reclass-offset:${id}:%`)]
  );
  const toAccountIds = new Set();
  const parsedOffsets = [];
  for (const row of offsetRows || []) {
    ...
    const toAccountId = parts[3];
    if (!jeId || !lineId || !toAccountId) continue;
    parsedOffsets.push({ jeId, lineId, toAccountId });
    toAccountIds.add(toAccountId);
  }

  // reclass-rules:<origJeId>:<dumpGlId>
  const ruleRows = await db.all(
    `SELECT id, memo FROM journal_entries
     WHERE entity_id = ? AND status = 'POSTED' AND reversed_by_je_id IS NULL
       AND (${jeIds.map(() => 'memo LIKE ?').join(' OR ')})
     ORDER BY posting_date ASC, created_at ASC, je_number ASC`,
    [entityId, ...jeIds.map((id) => `reclass-rules:${id}:%`)]
  );
  const ruleByKey = new Map(); // `${jeId}:${glId}` → rclsJeId
  const rclsJeIds = [];
  for (const row of ruleRows || []) {
    ...
    const jeId = parts[1];
    const glId = parts[2];
    if (!jeId || !glId) continue;
    ruleByKey.set(`${jeId}:${glId}`, row.id);
    rclsJeIds.push(row.id);
  }

  // Target account on each RCLS-RULE JE = the non-clearing expense/income line
  // (debit for charges that left a dump, or credit for reverse).
  const rclsTarget = new Map(); // rclsJeId → { id, number, name }
  const rclsLineIds = new Map(); // rclsJeId → expense line id (for chained reclass-offset)
  if (rclsJeIds.length) {
    const rph = rclsJeIds.map(() => '?').join(',');
    const rclsLines = await db.all(
      `SELECT jel.id AS line_id, jel.journal_entry_id, jel.account_id,
       ...
       FROM journal_entry_lines jel
       JOIN accounts a ON a.id = jel.account_id
       WHERE jel.journal_entry_id IN (${rph})`,
      rclsJeIds
    );
    const byRcls = new Map();
    for (const ln of rclsLines || []) {
      const list = byRcls.get(ln.journal_entry_id) || [];
      list.push(ln);
      byRcls.set(ln.journal_entry_id, list);
    }
    for (const [rclsId, lines] of byRcls) {
      // Prefer EXPENSE/INCOME/COGS; fall back to largest absolute line
      const prefer = lines.filter((l) => /EXPENSE|INCOME|COST OF GOODS/i.test(String(l.account_type || '')));
      const pool = prefer.length ? prefer : lines;
      pool.sort((a, b) => (Math.abs(b.debit - b.credit) - Math.abs(a.debit - a.credit)));
      const pick = pool[0];
      if (!pick) continue;
      rclsTarget.set(rclsId, {
        id: pick.account_id,
        number: pick.account_number,
        name: pick.account_name,
      });
      rclsLineIds.set(rclsId, pick.line_id);
      toAccountIds.add(pick.account_id);
    }

    // Chained reclass-offset on the RCLS JE itself (e.g. 5910 → 5500 fix)
    if (rclsJeIds.length) {
      const chainRows = await db.all(
        `SELECT memo FROM journal_entries
         WHERE entity_id = ? AND status = 'POSTED' AND reversed_by_je_id IS NULL
           AND (${rclsJeIds.map(() => 'memo LIKE ?').join(' OR ')})
         ORDER BY posting_date ASC, created_at ASC, je_number ASC`,
        [entityId, ...rclsJeIds.map((id) => `reclass-offset:${id}:%`)]
      );
      for (const row of chainRows || []) {
        const parts = String(row.memo || '').split(':');
        if (parts.length < 4) continue;
        const rclsId = parts[1];
        const lineId = parts[2];
        const toAccountId = parts[3];
        if (rclsLineIds.get(rclsId) !== lineId) continue;
        parsedOffsets.push({ jeId: rclsId, lineId, toAccountId, isRclsChain: true });
        toAccountIds.add(toAccountId);
      }
    }
  }
  ...
  if (toAccountIds.size) {
    const aph = [...toAccountIds].map(() => '?').join(',');
    const accts = await db.all(
      `SELECT id, account_number, account_name FROM accounts WHERE id IN (${aph})`,
      [...toAccountIds]
    );
    for (const a of accts || []) acctById.set(a.id, a);
  }
  for (const po of parsedOffsets) {
    const acct = acctById.get(po.toAccountId);
    if (!acct) continue;
    const key = `${po.jeId}:${po.lineId}`;
    offsetTarget.set(key, {
      id: acct.id,
      number: acct.account_number,
      name: acct.account_name,
    });
    if (po.isRclsChain) {
      rclsTarget.set(po.jeId, {
        id: acct.id,
        number: acct.account_number,
        name: acct.account_name,
      });
  ...
  const leaf = (name) => {
    ...
    return (i >= 0 ? s.slice(i + 1) : s).trim() || s;
  };

  return entries.map((e) => {
    const offsets = byJe.get(e.journal_entry_id) || [];
    if (!offsets.length) {
      return {
        ...e,
        category_account_number: null,
        category_account_name: null,
        category_label: null,
        category_is_split: false,
      };
    }

    const material = offsets.filter((o) => Math.abs(Number(o.debit) - Number(o.credit)) >= 0.005);
    const pool = material.length ? material : offsets;
    const prefer = pool.filter((o) => /EXPENSE|INCOME|COST OF GOODS|EQUITY/i.test(String(o.account_type || '')));
    const ranked = (prefer.length ? prefer : pool)
      .slice()
    ...
    for (const o of ranked) {
      ...
      const viaOffset = offsetTarget.get(`${e.journal_entry_id}:${o.line_id}`);
      if (viaOffset) {
        acct = viaOffset;
      } else if (o.gl_id) {
        const rclsId = ruleByKey.get(`${e.journal_entry_id}:${o.gl_id}`);
        if (rclsId && rclsTarget.has(rclsId)) {
          acct = rclsTarget.get(rclsId);
        }
      }
      resolved.push(acct);
    }

    // Unique by account number
    const uniq = [];
    const seen = new Set();
    ...
    const isSplit = uniq.length > 1;
    const primary = uniq[0] || null;
    const label = !primary
      ? null
      : isSplit
        ? `${primary.number} ${leaf(primary.name)} +${uniq.length - 1}`
        : `${primary.number} ${leaf(primary.name)}`;

    return {
      ...e,
      category_account_number: primary?.number || null,
      category_account_name: primary?.name || null,
...
export async function buildWorksheet(db, {
  ...
  const requestDate = normalizeIsoDate(statementDate) || String(statementDate || '').slice(0, 10);
  let session = await getSessionForPeriod(db, entityId, accountId, requestDate);
  // Simmons-style cycles end on the 1st (Jan thru 2/01). The UI/URL may carry
  // that thru-date while the OPEN session is keyed to month-end (1/31). Resume
  // the in-cycle OPEN session so begin/end stay on the real statement figures.
  if (!session) {
    const peek = peekBundledStatement(account.account_number, requestDate, entityId);
    const cycleStart = peek?.meta?.periodStart;
    const cycleEnd = peek?.meta?.periodEnd;
    if (cycleStart && cycleEnd && requestDate >= cycleStart && requestDate <= cycleEnd) {
      session = await db.get(
        `SELECT * FROM bank_reconciliation_sessions
         WHERE entity_id = ? AND account_id = ? AND status = 'OPEN'
           AND statement_date >= ? AND statement_date <= ?
         ORDER BY statement_date ASC LIMIT 1`,
        [entityId, accountId, cycleStart, cycleEnd]
      );
    }
  }
  const sessionKeyDate = session
    ? (normalizeIsoDate(session.statement_date) || String(session.statement_date).slice(0, 10))
    : requestDate;
  ...
  // Populate register from discovered statement PDF before loading uncleared lines.
  let registerImportResult = null;
  try {
    registerImportResult = await ensureStatementRegisterFromPdf(db, {
      entityId,
      accountId,
      accountNumber: account.account_number,
      statementDate: requestDate,
      userId,
    });
    if (registerImportResult?.error) {
      console.warn('worksheet register import error:', registerImportResult.error);
    } else if (registerImportResult?.posted > 0) {
      console.info(`worksheet register import: posted ${registerImportResult.posted} line(s)`);
    } else if (registerImportResult?.skipped) {
      console.info('worksheet register import skipped:', registerImportResult.reason);
    }
  } catch (err) {
    console.warn('worksheet statement register import (non-fatal):', err.message);
  }
  ...
  //
  // YE reclass / cutover AJEs that touch cash are NOT bank activity — exclude them
  // so they never appear as Deposits/Payments on the statement recon.
  const nonBank = sqlExcludeNonBankReconJournals('je');
  const entries = await db.all(
    `SELECT gl.id, gl.journal_entry_id, gl.posting_date, gl.debit, gl.credit, gl.description, gl.reconciliation_status,
            je.je_number, je.description AS je_description, je.source AS je_source
     FROM general_ledger gl
    ...
    [entityId, accountId, ...nonBank.params]
  );
  ...
  // Never let a stale OPEN session beginning (e.g. LJC $11,450.19 pasted onto OMC)
  // win over the entity's own statement previous balance.
  const displayBeginning = session?.status === 'CLOSED' && session?.beginning_balance != null
    ? round2(session.beginning_balance)
    : (stmtMetaFinal.previousBalance != null
      ? round2(stmtMetaFinal.previousBalance)
      : (priorClosedSession
        ? beginningBalance
        : beginningBalance));
  // OPEN ending: statement currentBalance is SOT. Stale OPEN session endings
  // (e.g. Simmons+b1 rollup $2,023.18 vs …7036 PDF $1,983.97) must not stick.
  const suggestedEnding = session?.status === 'CLOSED' && session?.ending_balance != null
    ? round2(session.ending_balance)
    : (stmtMetaFinal.currentBalance != null
      ? round2(stmtMetaFinal.currentBalance)
      : (session?.ending_balance != null ? round2(session.ending_balance) : null));

  // Correct a bad OPEN session row in place so resume-open / Monthly Books / UI
  // that still read periodSession.beginningBalance see the statement figures.
  if (
    session
    && session.status === 'OPEN'
    && (
      (stmtMetaFinal.previousBalance != null
        && Math.abs(round2(session.beginning_balance) - round2(stmtMetaFinal.previousBalance)) >= 0.01)
      || (stmtMetaFinal.currentBalance != null
        && Math.abs(round2(session.ending_balance) - round2(stmtMetaFinal.currentBalance)) >= 0.01)
    )
  ) {
    const fixBegin = stmtMetaFinal.previousBalance != null
      ? round2(stmtMetaFinal.previousBalance)
      : round2(session.beginning_balance);
    const fixEnd = stmtMetaFinal.currentBalance != null
      ? round2(stmtMetaFinal.currentBalance)
      : round2(session.ending_balance);
    // With 0 cleared lines, Difference = ending − beginning (not a fake $0.00).
    const lineCount = await countSessionLines(db, session.id);
    const fixDiff = lineCount === 0
      ? round2(fixEnd - fixBegin)
      : round2(session.difference);
    await db.run(
      `UPDATE bank_reconciliation_sessions
       SET beginning_balance = ?, ending_balance = ?, difference = ?,
           notes = COALESCE(notes, '') || ?
      ...
      [
        ...
        fixEnd,
        fixDiff,
        ` | Auto-corrected begin/end from statement PDF (${fixBegin} → ${fixEnd})`,
        session.id,
      ]
    );
    session.beginning_balance = fixBegin;
    session.ending_balance = fixEnd;
    session.difference = fixDiff;
    if (periodSession) {
      periodSession.beginningBalance = fixBegin;
      periodSession.endingBalance = fixEnd;
      periodSession.difference = fixDiff;
    }
  }

  // Heal stale OPEN difference=0 when nothing is cleared yet (begin≠end).
  if (session && session.status === 'OPEN') {
    const lineCount = Number(await countSessionLines(db, session.id)) || 0;
    if (lineCount === 0) {
      const expectedDiff = round2(
        round2(session.ending_balance) - round2(session.beginning_balance)
      );
      if (Math.abs(round2(session.difference) - expectedDiff) >= 0.01) {
        await db.run(
          `UPDATE bank_reconciliation_sessions SET difference = ? WHERE id = ? AND status = 'OPEN'`,
          [expectedDiff, session.id]
        );
        session.difference = expectedDiff;
        if (periodSession) periodSession.difference = expectedDiff;
      }
    }
  }
  ...
  const mergedRaw = [...openEntries, ...reconciledDecorated]
    .sort((a, b) => String(a.posting_date).localeCompare(String(b.posting_date)));
  let mergedEntries = mergedRaw;
  try {
    mergedEntries = await enrichEntriesWithCategory(db, entityId, accountId, mergedRaw);
  } catch (err) {
    console.warn('enrichEntriesWithCategory failed:', err?.message || err);
    mergedEntries = mergedRaw;
  }
  ...
  // Discover/persist the statement PDF so the side-by-side viewer has it even
  // when Jerry did not manually upload (cloud deploy of data/bank-imports/…).
  let statementFileAttached = false;
  try {
    const file = await resolveStatementFile(db, {
      entityId,
      accountId,
      accountNumber: account.account_number,
      statementDate,
    ...
    statementFileAttached = !!(file?.file_data);
  } catch (err) {
    console.warn('resolveStatementFile during worksheet:', err?.message || err);
  }

  // Worksheet key date = OPEN/CLOSED session date (or the requested date).
  // Never rewrite it to statement periodEnd (2/01) — that broke the URL/reload
  // path and folded the whole cycle into beginning (begin = end = Current).
  const worksheetKeyDate = sessionKeyDate || requestDate || effectiveAsOf;

  return {
    account,
    statementDate: worksheetKeyDate,
    beginningBalance: displayBeginning,
    displayBeginning,
...
async function countSessionLines(db, sessionId) {
  ...
  // PG returns COUNT as string — always coerce so `=== 0` heal/guards work.
...
export async function closeBankReconciliation(db, {
  ...
  if (glIds.length === 0) {
    ...
    const nonBankUnc = sqlExcludeNonBankReconJournals('je');
    const uncParams = [entityId, accountId, ...nonBankUnc.params, statementDate];
    let uncSql = `
      SELECT COUNT(*) AS n
      FROM general_ledger gl
...
export async function reopenBankReconciliation(db, { entityId, accountId, statementDate }) {
  ...
  if (account) {
    const bundled = peekBundledStatement(account.account_number, statementDate, entityId);
    if (bundled?.meta?.previousBalance != null) {
...
export async function undoLastBankReconciliation(db, { entityId, accountId }) {
  ...
  for (const row of laterOpen || []) {
    ...
    let begin = null;
    if (account) {
      const bundled = peekBundledStatement(account.account_number, row.statement_date, entityId);
      if (bundled?.meta?.previousBalance != null) {
...
export async function undoBankReconsInRange(db, {
  ...
  for (const accountNumber of accountNumbers) {
    ...
    for (const row of laterOpen || []) {
      ...
      if (!sd) continue;
      const bundled = peekBundledStatement(account.account_number, sd, entityId);
...
export async function autoReconcileToTarget(db, {
  ...
  const nonBankAuto = sqlExcludeNonBankReconJournals('je');
  const params = [entityId, acc.id, ...nonBankAuto.params];
  let sql = `
