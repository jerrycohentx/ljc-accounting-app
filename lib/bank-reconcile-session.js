...
export async function getBeginningBalance(db, entityId, accountId, statementDate, normalBalance) {
  ...
  if (thisSession && thisSession.beginning_balance != null) {
    return round2(thisSession.beginning_balance);
  }

  const prior = await db.get(
    `SELECT ending_balance, statement_date FROM bank_reconciliation_sessions
...
export async function buildWorksheet(db, {
  ...
  // Session beginning is authoritative when present (fixes wrong $cleared-net as begin).
  const displayBeginning = session?.beginning_balance != null
