...
export async function getBeginningBalance(db, entityId, accountId, statementDate, normalBalance) {
  ...
  if (prior && prior.ending_balance != null) return round2(prior.ending_balance);
...
export async function undoBankReconsInRange(db, {
  ...
  for (const accountNumber of accountNumbers) {
    ...
    const sessions = await db.all(
      ...
      [entityId, account.id, from, through]
    );
  ...
  return { entityId, fromMonth: from, throughMonth: through, accountNumbers, results };
}
