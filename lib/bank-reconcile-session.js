...
export async function getBeginningBalance(db, entityId, accountId, statementDate, normalBalance) {
  ...
  // Prefer an OPEN session's stored beginning for this statement (rebuild after reopen).
  const openSession = await db.get(
    `SELECT beginning_balance FROM bank_reconciliation_sessions
     WHERE entity_id = ? AND account_id = ? AND statement_date = ?
       AND ABS(COALESCE(beginning_balance, 0)) >= 0.01
     LIMIT 1`,
    [entityId, accountId, statementDate]
  );
  if (openSession) return round2(openSession.beginning_balance);
