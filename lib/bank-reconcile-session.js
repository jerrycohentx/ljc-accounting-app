...
import { matchStatementToRegister, persistImportAutoMatches } from './reconcile-auto-match.js';
...
/** PDF-imported statements may not tie to the header balance by a few cents. */
const RECON_CLOSE_TOLERANCE = 0.07;
...
export async function buildWorksheet(db, {
  ...
  const effectiveAsOf = stmtMeta.periodEnd || statementDate;

  const entries = await db.all(
    `SELECT gl.id, gl.posting_date, gl.debit, gl.credit, gl.description, gl.reconciliation_status,
    ...
    [entityId, accountId, effectiveAsOf]
  );
  ...
  const displayBeginning = stmtMetaFinal.previousBalance != null
  ...
  return {
    ...
    statementLines: statementView.lines,
    entries: entries || [],
    suggestedCheckedGlIds: autoMatchResult.suggestedCheckedGlIds,
    autoMatch: {
...
export async function closeBankReconciliation(db, {
  ...
  if (!Array.isArray(glIds) || glIds.length === 0) {
    throw new Error('Select at least one cleared transaction');
  }
  ...
  const account = await db.get(
    'SELECT id, normal_balance FROM accounts WHERE id = ? AND entity_id = ?',
    [accountId, entityId]
  );
  if (!account) throw new Error('Account not found');
  ...
  for (const row of clearedRows) {
    clearedNet = clearedNet.plus(signedGlDelta(row, account.normal_balance));
  }

  const ending = new Decimal(statementEndingBalance);
  const computed = new Decimal(beginningBalance).plus(clearedNet);
  const difference = round2(computed.minus(ending).toNumber());

  if (Math.abs(difference) >= RECON_CLOSE_TOLERANCE) {
    ...
    err.details = {
      beginningBalance,
      clearedNet: round2(clearedNet.toNumber()),
      computedEnding: round2(computed.toNumber()),
  ...
  return {
    sessionId,
    status: 'CLOSED',
    reconciledCount: clearedRows.length,
    beginningBalance,
