 ...
 * ent-ljc Simmons x0260 checking is Beancount 1030 (Cash:Simmons-Bank-Ckg-0260), not 1000.
 */
...
/** Entity-specific overrides (institution key → GL account number). */
const PLAID_BANK_ACCOUNT_BY_ENTITY = {
  'ent-ljc': PLAID_BANK_ACCOUNT_BY_INSTITUTION_KEY,
};

export function resolvePlaidBankAccountNumber(entityId, institutionKey) {
  const byEntity = PLAID_BANK_ACCOUNT_BY_ENTITY[entityId];
  if (byEntity && institutionKey && byEntity[institutionKey]) {
    return byEntity[institutionKey];
  }
  return institutionKey ? PLAID_BANK_ACCOUNT_BY_INSTITUTION_KEY[institutionKey] : null;
}
...
export async function syncPlaidItem(db, { entityId, itemId }) {
  ...
  return {
    ...
    bankAccountNumber: resolvePlaidBankAccountNumber(entityId, institutionKey),
    dateRange: {
      start: dates[0] || null,
...
export async function commitPlaidImport(db, session, userId) {
 ...
 */
export async function syncAndQueuePlaidItem(db, { entityId, itemId, userId }) {
...
export async function runPlaidAutoSync(db, { reason = 'scheduled', itemId = null, userId = null } = {}) {
  if (running) {
  ...
  try {
    await ensurePlaidSchema(db);
    const automationUserId = userId || await resolveAutomationUserId(db);
