...
import { ensurePaymentReturnSchema } from './payment-return-schema.js';
...
export async function detectPaymentReturn(db, {
  ...
  await db.run(
    `INSERT INTO payment_returns
     (id, entity_id, return_type, status, sync_status, amount_cents, description, bank_reference,
      bank_fitid, import_transaction_id, loan_id, loan_num, holdback_draw_id, match_rule_id, recorded_at)
     VALUES (?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      id,
      entityId,
...
export async function draftOrPostPaymentReturnJe(db, {
  ...
  const row = await db.get('SELECT * FROM payment_returns WHERE id = ? AND entity_id = ?', [returnId, entityId]);
  ...
  const amount = new Decimal(row.amount_cents).div(100);
  ...
  await db.run(
    `UPDATE payment_returns SET journal_entry_id = ?, status = 'matched', updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
    [jeId, returnId]
  );
...
export async function listPendingPaymentReturnSync(db, entityId = 'ent-ljc') {
  await ensurePaymentReturnSchema(db);
  const rows = await db.all(
    `SELECT id, entity_id, return_type, status, amount_cents, description, bank_reference,
            loan_id, loan_num, holdback_draw_id, recorded_at, created_at
  ...
  return rows.map((r) => ({
    ...
    loanId: r.loan_id,
    loanNum: r.loan_num,
    holdbackDrawId: r.holdback_draw_id,
...
export async function ackPaymentReturnSync(db, returnId) {
  ...
  if (!row) throw new Error('Payment return not found');
  await db.run(
    `UPDATE payment_returns SET sync_status = 'synced', synced_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
    [returnId]
  );
  ...
  const row = await db.get(
    `SELECT id FROM payment_returns
     WHERE entity_id = ? AND loan_id IS NULL AND holdback_draw_id IS NULL
       AND status = 'pending' AND ABS(amount_cents - ?) <= 1
