...
export async function detectPaymentReturn(db, {
  ...
  if (status === 'matched' && rule.autoPostWhenMatched) {
    const gate = checkAutoPostAllowed('payment return match');
    await draftOrPostPaymentReturnJe(db, {
      returnId: id,
      entityId,
      bankAccountNumber,
      userId,
      autoPost: gate.allowed,
    });
...
export async function draftOrPostPaymentReturnJe(db, {
  ...
  if (autoPost && row.import_transaction_id && (row.loan_id || row.holdback_draw_id)) {
    const gate = checkAutoPostAllowed('payment return auto-post');
    if (!gate.allowed) {
      return { journalEntryId: jeId, drafted: true, autoPostBlocked: true, message: gate.reason };
    }
