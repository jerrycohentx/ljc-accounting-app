...
/**
 * Tiny checkpoint for Save & Exit / Back Up Company.
 * Postgres already holds the live books — this only records that Jerry saved,
 * plus row counts. Never loads journal lines / GL / PDFs into memory.
 */
async function backupPostgresCheckpoint(reason) {
  const slug = timestampSlug();
  ...
  for (const table of [
    ...
    try {
      ...
      rowCounts[table] = Number(row?.c || 0);
    } catch {
  ...
  try {
    const row = await db.get(
      `SELECT COUNT(*) AS c FROM journal_entries WHERE status = 'DRAFT'`
    );
    draftCount = Number(row?.c || 0);
  } catch { /* ignore */ }
  ...
  const snapshot = {
    ...
    note: 'Lightweight Save & Exit checkpoint. Live ledger remains in PostgreSQL (PDFs not copied).',
  ...
  return {
    filename: `postgres-${slug}-${reason}-checkpoint.json`,
    content,
    databaseType: 'postgres-checkpoint',
    note: snapshot.note,
  };
}
...
async function backupPostgresJsonSnapshot(reason) {
  const slug = timestampSlug();
  const db = await getDatabase();
  // Omit general_ledger — it is derived from journal lines and is the biggest table.
  const tables = SNAPSHOT_TABLES.filter((t) => t !== 'general_ledger');
  const snapshot = {
    exportedAt: new Date().toISOString(),
    reason,
  ...
  let runningBytes = 2 * 1024; // envelope
  for (const table of tables) {
    try {
      const rows = await db.all(`SELECT * FROM ${table}`);
      const chunk = JSON.stringify(rows);
      runningBytes += Buffer.byteLength(chunk, 'utf8') + table.length + 8;
      if (runningBytes > MAX_BACKUP_BYTES) {
        snapshot.tables[table] = { truncated: true, rowCount: rows.length };
  ...
  if (Buffer.byteLength(content, 'utf8') > MAX_BACKUP_BYTES) {
    return backupPostgresCheckpoint(reason);
  }
  return {
    filename: `postgres-${slug}-${reason}.json`,
    content,
    databaseType: 'postgres-json',
    note: 'Ledger snapshot (PDFs/statement files and general_ledger excluded)',
  };
}
...
async function backupPostgresContent(reason) {
  ...
  // Default: tiny checkpoint. Full dumps previously OOM'd Render → HTTP 502.
  // Opt into heavier snapshots only with BACKUP_FULL_LEDGER=1.
  if (process.env.BACKUP_FULL_LEDGER !== '1') {
    return backupPostgresCheckpoint(reason);
  }

  const useFullDump = process.env.BACKUP_ALLOW_PG_DUMP === '1';
  if (!useFullDump) {
  ...
  try {
    ...
    const fallback = await backupPostgresCheckpoint(reason);
    fallback.note = `pg_dump unavailable (${err.message}); used checkpoint`;
    return fallback;
  }
}
