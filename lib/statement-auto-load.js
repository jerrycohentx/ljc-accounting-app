...
export function getStatementAutoLoadStatus() {
  const legacyOff = process.env.STATEMENT_AUTO_LOAD_ENABLED === '0';
  const forceOff = ['force-off', 'never'].includes(String(process.env.STATEMENT_AUTO_LOAD_ENABLED || '').toLowerCase());
  const enabled = !isRebuildFreezeActive() && !forceOff && !(legacyOff && process.env.HONOR_LEGACY_AUTOLOAD_OFF === '1');
  return {
    enabled,
...
export function startStatementAutoLoad(getDb) {
  const status = getStatementAutoLoadStatus();
  if (!status.enabled) {
    console.log('Statement auto-load disabled');
    return;
  }
