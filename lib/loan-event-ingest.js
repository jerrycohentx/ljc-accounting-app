...
import {
  isRelatedPartyIcEvent,
  draftRelatedPartyIntercompanyJournals,
} from './loan-event-journal.js';
...
export async function ingestLoanTrackerEvent(db, payload) {
  ...
  const existing = await db.get(
    'SELECT id, status, journal_entry_id FROM loan_tracker_events WHERE idempotency_key = ?',
    [key]
  );
  if (existing) {
  ...
  // Related-party prepaid / interest: draft mirrored IC journals (no cash).
  let draft = null;
  if (isRelatedPartyIcEvent(eventType)) {
    try {
      draft = await draftRelatedPartyIntercompanyJournals(db, { payload, eventId: id });
      if (draft?.ljc?.journalId) {
        await db.run(
          'UPDATE loan_tracker_events SET journal_entry_id = ?, status = ? WHERE id = ?',
          [draft.ljc.journalId, 'DRAFTED', id]
        );
      }
    } catch (err) {
      console.error('related-party IC draft failed:', err.message);
      await db.run(
        'UPDATE loan_tracker_events SET status = ? WHERE id = ?',
        ['ERROR', id]
      );
      throw err;
    }
    return {
      ...
      idempotencyKey: key,
      eventType,
      status: draft?.drafted ? 'DRAFTED' : 'PENDING',
      draft,
      note: 'Related-party intercompany drafts created for LJC and counterparty (no cash). Approve in Journals.',
    };
  }
