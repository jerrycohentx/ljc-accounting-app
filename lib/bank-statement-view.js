...
function loadJsonStatement(accountNumber, periodStart, periodEnd, statementDate = periodEnd) {
  ...
  let openingBalance = match.meta?.previousBalance != null ? round2(match.meta.previousBalance) : null;
  let periodLines = lines;
  let effectivePeriodStart = match.meta?.periodStart || periodStart;

  // A statement cycle can OPEN BEFORE the period being reconciled. AMEX's
  // 12/10-01/09 cycle straddles the day LJC's books start (2026-01-01), and a
  // prior close landing mid-cycle does the same. Activity before periodStart is
  // already baked into the book's opening balance, so it must not be presented
  // as reconcilable: left in, the worksheet showed 45 December lines as "on
  // statement only" (inviting 45 double-counting entries) and used the
  // statement's own previousBalance -- a 12/09 balance the books never saw --
  // as the beginning. Roll previousBalance forward through those pre-period
  // lines to get the true balance AT periodStart, then drop them.
  const priorLines = periodStart ? lines.filter((l) => l.date && l.date < periodStart) : [];
  if (priorLines.length && openingBalance != null) {
    const priorDelta = priorLines.reduce((sum, l) => sum + l.amount, 0);
    openingBalance = round2(openingBalance + priorDelta);
    periodLines = lines.filter((l) => !(l.date && l.date < periodStart));
  ...
  return {
    meta: {
      ...
      priorPeriodLinesFolded: priorLines.length || undefined,
      statementCycleStart: priorLines.length ? (match.meta?.periodStart || null) : undefined,
    },
    lines: periodLines,
  };
}
