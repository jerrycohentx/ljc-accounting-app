...
/** Bundled statement JSON is per-entity. Sister entities must never inherit LJC cycles. */
const STATEMENT_JSON_BY_ENTITY_ACCOUNT = {
  'ent-ljc': {
...
function statementJsonRel(entityId, accountNumber) {
  const byEntity = STATEMENT_JSON_BY_ENTITY_ACCOUNT[entityId || 'ent-ljc'];
...
export async function resolveStatementPeriod(db, { entityId, accountId, statementDate }) {
  ...
  // Ignore $0 cutover stubs — they are not a real prior statement close and must
  // not move periodStart to Dec or block folding the first Amex cycle into books.
  const priorIsReal = prior && Math.abs(Number(prior.ending_balance) || 0) >= 0.01;
  const priorEnd = priorIsReal ? normalizeIsoDate(prior.statement_date) : null;
...
/**
 * The next statement ON FILE after a given date — the correct "what should I
 * reconcile next" answer for ANY account type. A month-end guess is wrong for
 * cards (AMEX closes the 6th–9th) and for banks whose cycles spill (Simmons Jan
 * runs 1/01–2/01). Picks the earliest bundled statement whose period STARTS
 * after `afterISO` (period start, not end: Simmons' Feb statement ENDS 3/01,
 * after its session closed at 2/28 — comparing ends would re-suggest the
 * period that was just reconciled).
 */
export function nextBundledStatementEnd(accountNumber, afterISO, entityId = 'ent-ljc') {
  const rel = statementJsonRel(entityId, accountNumber);
  if (!rel || !afterISO) return null;
  const filePath = path.join(ROOT, rel);
  if (!fs.existsSync(filePath)) return null;
  const data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
  const after = String(afterISO).slice(0, 10);
  const candidates = (data.statements || [])
    .map((s) => {
      const end = statementPeriodEnd(s.meta);
      if (!end) return null;
      const start = s.meta?.periodStart || `${end.slice(0, 7)}-01`;
      return { start, end };
    })
    .filter((s) => s && s.start > after)
    .sort((a, b) => a.start.localeCompare(b.start));
  return candidates.length ? candidates[0].end : null;
}

/**
 * When Jerry (or the URL) uses the statement's thru-date as statementDate
 * (Simmons Jan ends 2/01), do NOT treat that date as "first of February" and
 * fold the whole January cycle into beginning — that makes begin = end and
 * Difference $0.00 with 0 cleared lines.
 */
function cyclePeriodStartForRequest(match, statementDate, fallbackStart) {
  const sd = normalizeIsoDate(statementDate) || String(statementDate || '').slice(0, 10);
  const end = statementPeriodEnd(match?.meta);
  const cycleStart = match?.meta?.periodStart || null;
  if (sd && end && sd === end && cycleStart) return cycleStart;
  return fallbackStart;
}

/** Preview bundled statement meta without DB (for prepare step). */
export function peekBundledStatement(accountNumber, statementDate = null, entityId = 'ent-ljc') {
  const rel = statementJsonRel(entityId, accountNumber);
  if (!rel) return null;
  const filePath = path.join(ROOT, rel);
  if (!fs.existsSync(filePath)) return null;
  ...
  const match = findJsonStatement(statements, sd, calendarStart, sd);
  if (!match) return null;

  const end = statementPeriodEnd(match.meta);
  const periodStart = cyclePeriodStartForRequest(match, sd, calendarStart);
  // Amex: never fold — prepare beginning must match the PDF previous balance.
  const allowFold = accountNumber !== '2010';
  ...
  return {
    ...
    lineCount: (match.transactions || []).filter((t) => !(allowFold && periodStart && t.date && t.date < periodStart)).length,
  };
}

function loadJsonStatement(accountNumber, periodStart, periodEnd, statementDate = periodEnd, allowFold = true, entityId = 'ent-ljc') {
  const rel = statementJsonRel(entityId, accountNumber);
  if (!rel) return null;
  const filePath = path.join(ROOT, rel);
  if (!fs.existsSync(filePath)) return null;
 ...
 *
 * When a bundled statement JSON exists for the cycle (Amex/Simmons/Lone Star),
 * that file is authoritative for previous/ending balances and line list — it is
 * the extracted PDF. import_transactions may contain extras from overlapping
 * uploads; those must not inflate Payments/Charges vs the PDF Jerry is reading.
 * Import rows still supply matchedGlId / journal links by fitid.
 */
export async function getBankStatementView(db, { entityId, accountId, accountNumber, statementDate }) {
  const resolved = await resolveStatementPeriod(db, {
    entityId,
    accountId,
    statementDate,
  });
  let { periodStart, periodEnd, hasPriorClosedSession } = resolved;

  // If the request date is a statement's thru-date (e.g. Simmons 2/01), keep the
  // cycle's own start so January activity is not folded into beginning.
  const peek = accountNumber
    ? peekBundledStatement(accountNumber, statementDate, entityId)
    : null;
  const reqIso = normalizeIsoDate(statementDate);
  if (
    peek?.meta?.periodEnd
    && reqIso
    && reqIso === peek.meta.periodEnd
    && peek.meta.periodStart
  ) {
    periodStart = peek.meta.periodStart;
    periodEnd = peek.meta.periodEnd;
  }
  ...
  const importByFitid = new Map();
  for (const row of importRows || []) {
    if (row.fitid) importByFitid.set(String(row.fitid), row);
  }
  ...
  // Amex (and any card whose PDF Jerry reads side-by-side): never fold December
  // activity into opening on the statement panel. Show the statement's own
  // previous balance and full cycle lines. Books adjustments (e.g. a Dec
  // summary JE) are reversed separately so the register can clear those lines.
  const allowFold = accountNumber === '2010' ? false : !hasPriorClosedSession;
  const jsonView = accountNumber
    ? loadJsonStatement(accountNumber, periodStart, periodEnd, statementDate, allowFold, entityId)
    : null;

  if (jsonView) {
    // Authoritative PDF/JSON cycle — replace bloated import line lists.
    lines = jsonView.lines.map((line) => {
      const imp = line.fitid ? importByFitid.get(String(line.fitid)) : null;
      if (!imp) return line;
      return {
        ...line,
        matchedGlId: imp.matched_to_gl_id || line.matchedGlId,
        journalEntryId: imp.journal_entry_id || line.journalEntryId,
        status: imp.status || line.status,
        source: 'statement-json+import',
      };
    });
    meta = {
      ...meta,
      ...jsonView.meta,
      source: (importRows || []).length ? 'statement-json+import' : jsonView.meta.source,
      lineCount: lines.length,
      importLineCount: (importRows || []).length || undefined,
    };
...
/** Load bundled statement JSON from repo (Render has no OneDrive PDFs). */
export function loadBundledStatementForImport(accountNumber, statementDate, entityId = 'ent-ljc') {
  const rel = statementJsonRel(entityId, accountNumber);
  if (!rel) return null;
  const filePath = path.join(ROOT, rel);
  if (!fs.existsSync(filePath)) return null;
  ...
  const sd = normalizeIsoDate(statementDate) || String(statementDate || '').slice(0, 10);
  const month = sd.slice(0, 7);
  const match = findJsonStatement(statements, sd, `${month}-01`, sd)
    || statements.find((s) => statementPeriodEnd(s.meta) === sd);
  if (!match?.transactions?.length) return null;
  return {
    file: match.file || path.basename(rel),
    meta: { ...(match.meta || {}) },
