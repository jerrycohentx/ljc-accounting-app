...
export async function seedCsbJan2026(db, {
  ...
  // Align the existing OFX $93.80 credit to Check 1107252 (books + payee label).
  // OFX imported it as a cash credit (correct side for the check) but labeled it
  // "DEPOSIT - MOBILE", so the reconcile payee looked like a deposit and the
  // check appeared "missing" from Checks and Payments.
  const ofx93 = await db.get(
    `SELECT gl.id AS gl_id, gl.journal_entry_id, je.je_number, je.description AS je_description
     FROM general_ledger gl
     JOIN journal_entries je ON je.id = gl.journal_entry_id
     WHERE gl.entity_id = ? AND gl.account_id = ?
       AND ABS(gl.credit - 93.80) < 0.005
       AND je.status = 'POSTED'
       AND je.reversed_by_je_id IS NULL AND je.reverses_je_id IS NULL
     ORDER BY CASE
       WHEN je.description LIKE '%DEPOSIT - MOBILE%' OR gl.description LIKE '%DEPOSIT - MOBILE%' THEN 0
       WHEN je.description LIKE '%1107252%' OR gl.description LIKE '%1107252%' THEN 1
       ELSE 2
     END
     LIMIT 1`,
    [entityId, cash.id]
  );
  let check1107252 = { skipped: true };
  if (ofx93?.journal_entry_id) {
    await db.run(
      `UPDATE general_ledger
       SET description = 'CSB Check 1107252'
       WHERE id = ? AND entity_id = ?`,
      [ofx93.gl_id, entityId]
    );
    await db.run(
      `UPDATE journal_entries
       SET description = 'CSB Check 1107252',
           memo = COALESCE(NULLIF(memo, ''), 'From CSB account history PDF')
       WHERE id = ? AND entity_id = ?`,
      [ofx93.journal_entry_id, entityId]
    );
    // Prefer a stable check JE number when the OFX import id is still in place.
    if (ofx93.je_number && String(ofx93.je_number).startsWith('IMP-')) {
      const taken = await db.get(
        `SELECT id FROM journal_entries WHERE entity_id = ? AND je_number = ? AND id <> ?`,
        [entityId, 'CSB-1385-CHK-1107252', ofx93.journal_entry_id]
      );
      if (!taken) {
        await db.run(
          `UPDATE journal_entries SET je_number = ? WHERE id = ? AND entity_id = ?`,
          ['CSB-1385-CHK-1107252', ofx93.journal_entry_id, entityId]
        );
      }
    }
    check1107252 = {
      skipped: false,
      glId: ofx93.gl_id,
      journalEntryId: ofx93.journal_entry_id,
      priorJeNumber: ofx93.je_number,
    };
  }
