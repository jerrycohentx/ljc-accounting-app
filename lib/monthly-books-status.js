...
function sessionStatusPayload(session, account, displayName, target) {
  const statementDate =
    normalizeIsoDate(session.statement_date)
    || String(session.statement_date || '').slice(0, 10)
    || target?.statementDate
    || null;

  return { session, displayName, target, statementDate };
}

async function classifySession(db, account, { session, displayName, target, statementDate }) {
  ...
  if (balanced) {
    return {
      ...
      statementDate,
      statementLabel: target?.label || null,
      difference: 0,
    };
  }
  ...
  return {
    ...
    statementDate,
    statementLabel: target?.label || null,
    endingBalance: target?.endingBalance ?? Number(session.ending_balance),
    difference: diff,
  };
}
...
async function reconAccountStatus(db, entityId, account, year, month, periodStart, periodEnd) {
  ...
  // Prefer configured statement target; if missing, still surface any OPEN/CLOSED
  // session that covers this calendar month (integrity already finds these).
  let session = null;
  if (target?.statementDate) {
    session = await getSessionForPeriod(db, entityId, account.id, target.statementDate);
  }
  if (!session) {
    session = await findCoveringSession(db, entityId, account.id, periodStart, periodEnd);
  }
  ...
  return classifySession(
    db,
    account,
    sessionStatusPayload(session, account, displayName, target)
  );
}
...
export async function getMonthlyBooksStatus(db, { entityId, year, month }) {
  ...
  // Prefer earliest OPEN recon month over calendar "today" (Aug with no data).
  let suggestedWorkingMonth = m;
  let suggestedWorkingYear = y;
  try {
    const open = await getEarliestOpenSessionForEntity(db, entityId, { year: y });
    const openDate = normalizeIsoDate(open?.statement_date);
    if (openDate) {
      suggestedWorkingYear = Number(openDate.slice(0, 4));
      suggestedWorkingMonth = Number(openDate.slice(5, 7));
    } else if (y === 2026) {
      // No OPEN session yet — default Home to January, not the empty calendar month.
      suggestedWorkingMonth = 1;
      suggestedWorkingYear = 2026;
    }
  } catch {
    if (y === 2026) {
      suggestedWorkingMonth = 1;
  ...
  return {
    ...
    periodStart,
    periodEnd,
    suggestedWorkingYear,
    suggestedWorkingMonth,
    steps: {
      downloads: {
        key: 'downloads',
      ...
      reconcile: {
        ...
        statusLabel: allReconClosed && anyReconStarted
          ...
          : reconAccounts.some((a) => a.status === 'in_progress' || a.status === 'needs_fix')
            ? 'Open reconciliation in progress — click to resume'
