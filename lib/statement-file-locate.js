...
function hayIncludesToken(hay, token) {
  const t = String(token || '').toLowerCase().trim();
  if (!t) return false;
  if (/^\d{3,5}$/.test(t)) {
    // Last-4 / account suffixes — avoid matching inside YYYYMMDD (20260118 ⊃ 0260).
...
export function validateStatementPdfPath(filePath, entityId, accountNumber) {
  ...
  if (spec.kind === 'cc') {
    ...
    const hasHint = (spec.hints || []).some((h) => hayIncludesToken(hay, h) || hay.includes(String(h).toLowerCase()));
...
function findPdfOnDisk(accountNumber, statementDate, entityId = null) {
  ...
  const exactName = bundled?.meta?.label || bundled?.file || null;
  const hints = pdfHintsFor(entityId, accountNumber);
  const spec = accountSpec(entityId, accountNumber);

  // Repo-shipped PDF next to bundled JSON (Render has no OneDrive).
  if (bundled?.file) {
    for (const dir of [
      path.join(ROOT, 'data/bank-imports/OMC/chase'),
      path.join(ROOT, 'data/bank-imports/OMC'),
      path.join(ROOT, 'data/bank-imports/LJC/amex'),
      path.join(ROOT, 'data/bank-imports/LJC/statements'),
    ]) {
      const candidate = path.join(dir, bundled.file);
      if (!fs.existsSync(candidate)) continue;
      const valid = validateStatementPdfPath(candidate, entityId, accountNumber);
...
async function purgeWrongStoredStatement(db, { entityId, accountId, statementDate, accountNumber }) {
  ...
  for (const row of rows || []) {
    ...
    if (!sd) {
      await db.run(
        'DELETE FROM bank_statement_files WHERE entity_id = ? AND account_id = ? AND file_name = ?',
        [entityId, accountId, row.file_name || '']
      );
    } else {
      await db.run(
        'DELETE FROM bank_statement_files WHERE entity_id = ? AND account_id = ? AND statement_date = ?',
        [entityId, accountId, sd]
      );
    }
    console.warn(
