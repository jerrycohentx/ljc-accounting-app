...
function postingDateIso(value) {
  // Prefer the calendar YYYY-MM-DD prefix from strings so PG timestamptz
  // values do not shift a day via Date#toISOString UTC conversion.
...
export async function selectSimmonsClearedIds(db, {
  ...
  let sql = `
    SELECT gl.id, gl.debit, gl.credit, gl.posting_date, gl.description,
           ...
           je.reversed_by_je_id, je.reverses_je_id
  ...
  for (const row of raw || []) {
    // Prefer isNoiseJe over JE reverse FKs — some restored OFX rows still carry
    // reverse links and would otherwise leave statement duplicates unmatched.
    if (isNoiseJe(row.je_number, row.je_description || row.description)) continue;
    if (row.reverses_je_id && /^REV-/i.test(String(row.je_number || ''))) continue;
    ...
    candidates.push({
      ...
      date: postingDateIso(row.posting_date),
      prefer: preferLabel(row.je_description || row.description),
      jeNumber: String(row.je_number || ''),
      description: String(row.je_description || row.description || ''),
      used: false,
    });
  }
  ...
  // If both RESTORE-IMP-… and the original IMP-… remain uncleared, keep RESTORE only.
  const restoreOrigIds = new Set();
  for (const c of filtered) {
    const m = c.jeNumber.match(/^RESTORE-(IMP-.+)$/i);
    if (m) restoreOrigIds.add(m[1]);
  }
  if (restoreOrigIds.size) {
    filtered = filtered.filter((c) => !restoreOrigIds.has(c.jeNumber));
  }
  ...
  if (statementTxns?.length) {
    ...
    for (const t of statementTxns) {
      const date = postingDateIso(t.date) || normalizeIsoDate(t.date);
      const cents = Math.round(signedStatementAmount(t) * 100);
      let bestIdx = -1;
      let bestPrefer = -1;
      for (let i = 0; i < pool.length; i++) {
        const c = pool[i];
        if (c.used || c.cents !== cents || c.date !== date) continue;
  ...
  for (const target of targets) {
    ...
    if (!pick.glIds?.length) {
      results.push({
        ...
        reconciled: false,
        pick,
        message: 'Could not match statement lines to GL — stopping so later months cannot steal lines',
      });
      break;
    }
