...
export async function selectSimmonsClearedIds(db, {
  ...
  if (statementTxns?.length) {
    ...
    for (const t of statementTxns) {
      ...
      const dateCandidates = [date, addDaysIso(date, -1), addDaysIso(date, 1)];
      let bestIdx = -1;
      let bestPrefer = -1;
      let bestDatePenalty = 99;
      for (let i = 0; i < pool.length; i++) {
        const c = pool[i];
        if (c.used || c.cents !== cents) continue;
        const di = dateCandidates.indexOf(c.date);
        if (di < 0) continue;
        // Exact date wins; then higher prefer label.
        if (di < bestDatePenalty || (di === bestDatePenalty && c.prefer > bestPrefer)) {
          bestDatePenalty = di;
          bestPrefer = c.prefer;
          bestIdx = i;
        }
