...
/** Meet-in-the-middle subset sum; caps at 36 items (2^18 per half). */
function findSubsetSumCents(items, needCents) {
  if (!items.length) return needCents === 0 ? [] : null;
  let list = items.filter((i) => i.cents !== 0);
  if (list.length > 36) {
    list = [...list].sort((a, b) => Math.abs(b.cents) - Math.abs(a.cents)).slice(0, 36);
  }
  ...
  if (statementTxns?.length) {
    ...
    for (const t of statementTxns) {
      ...
      const idx = pool.findIndex(
        (c) => !c.used && c.date === date && c.cents === cents
      );
      if (idx < 0) continue;
      pool[idx].used = true;
  ...
  for (const target of targets) {
    ...
    if (!pick.glIds?.length) {
      const r = await autoReconcileToTarget(db, {
        ...
        endingBalance: target.endingBalance,
        userId,
        notes: `Rebuild Simmons ${target.statementDate} (auto fallback)`,
        clearedAfterDate: prevStatementDate,
