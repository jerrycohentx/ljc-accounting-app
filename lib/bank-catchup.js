...
export async function reconcileToTarget(db, {
  ...
  const balanceExpr = acc.normal_balance === 'CREDIT' ? '(credit - debit)' : '(debit - credit)';
  ...
  const beginning = await db.get(
    `SELECT COALESCE(SUM(${balanceExpr}), 0) AS bal FROM general_ledger
  ...
  for (const e of entries) {
    const delta = acc.normal_balance === 'CREDIT'
      ? new Decimal(e.credit || 0).minus(e.debit || 0)
      : new Decimal(e.debit || 0).minus(e.credit || 0);
