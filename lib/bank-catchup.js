...
export async function getPostedBankBalance(db, entityId, accountNumber, asOfDate = null) {
  ...
  return { accountId: acc.id, accountNumber, balance: bal.toNumber() };
}
...
export async function postAllPendingImports(db, entityId, userId = DEMO_USER) {
  const gate = checkAutoPostAllowed('postAllPendingImports');
  if (!gate.allowed) {
...
/** @deprecated use autoReconcileToTarget from bank-reconcile-session.js */
export async function reconcileToTarget(db, opts) {
  return autoReconcileToTarget(db, opts);
}
