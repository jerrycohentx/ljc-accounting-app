...
export async function reverseJournalEntry(db, {
  ...
  const postingDate = normalizeIsoDate(reversalDate || original.posting_date)
    || String(reversalDate || original.posting_date || '').slice(0, 10);
  ...
  // Never copy the original description into the reversal — plug / tie-out language
  // in the source JE would block posting via assertNotPlugJournal. Reversing a plug
  // must always be allowed (that is how plugs are removed from the active books).
  const description = memo
    || `Offset of ${original.je_number} (removed from active register)`;
  ...
  await db.run(
    ...
    [
      ...
      postingDate,
      userId,
      memo || `AUTO-REVERSAL of ${original.je_number}`,
