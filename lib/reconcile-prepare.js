...
import { getBankStatementView, peekBundledStatement, nextBundledStatementEnd, normalizeIsoDate } from './bank-statement-view.js';
...
export async function prepareReconciliation(db, {
  ...
  // THE RULE (Jerry, 2026-07-16): the default statement date is the period
  // immediately after the LAST COMPLETED RECONCILIATION — for every account
  // type. In order of authority:
  //   1. the next statement ON FILE after the last close (right for card
  //      cycles ending the 6th–9th and for bank cycles that spill months);
  //   2. the next configured reconciliation target;
  //   3. the end of the following calendar month.
  //
  // normalizeIsoDate matters here: Postgres returns statement_date as a JS
  // Date, and String(date).slice(0,10) yields "Sat Feb 28" — which
  // nextStatementEnd could not parse, silently nulling the suggestion so the
  // screen fell through to the NEWEST bundled statement (May) instead of the
  // next unreconciled one (March). That is the bug Jerry hit.
  ...
  if (lastReconciledDate) {
    const nextOnFile = nextBundledStatementEnd(account.account_number, lastReconciledDate);
  ...
  if (importFromFolder) {
    ...
    if (ofxPath && fs.existsSync(ofxPath)) {
      try {
        ...
        // Reconcile-context load: statement LINES only, never journal entries
        // (and certainly never auto-POSTED ones, which this call once did).
        // Entries belong to Bank Import; prepare only needs the statement side.
