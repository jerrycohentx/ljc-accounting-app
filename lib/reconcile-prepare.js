...
export async function prepareReconciliation(db, {
  ...
  // Auto-import discovered statement PDF → posted register lines (Chase CC, etc.).
  let registerImport = null;
  try {
    registerImport = await ensureStatementRegisterFromPdf(db, {
      entityId,
      accountId,
      accountNumber: account.account_number,
      statementDate: suggestedStatementDate,
    ...
    if (registerImport?.posted > 0) {
      message += ` — ${registerImport.posted} charge(s) posted from statement`;
    } else if (registerImport?.imported > 0 && !registerImport.skipped) {
      message += ` — ${registerImport.imported} statement line(s) imported`;
    }
  } catch (err) {
    console.warn('prepare statement register import (non-fatal):', err.message);
  }
