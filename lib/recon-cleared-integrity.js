  ...
  if (storedDiffCents !== 0) {
    return {
      ...
      issue: {
        ...
        message: `Account ${account.account_number} reconciliation for ${String(session.statement_date).slice(0, 10)} is CLOSED but stored difference is ${fromCents(storedDiffCents).toFixed(2)} (must be $0.00).`,
      },
    };
  }
  ...
  if (liveDiffCents !== 0) {
    return {
      ...
      issue: {
        ...
        message:
          `Account ${account.account_number} (${account.account_name}) statement ${String(session.statement_date).slice(0, 10)}: ` +
...
export async function repairOrphanReconciledInClosedPeriods(db, { entityId, accountId = null } = {}) {
  ...
  for (const session of sessions || []) {
    const statementDate = String(session.statement_date).slice(0, 10);
