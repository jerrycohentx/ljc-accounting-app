 ...
 *
 * Creates a DRAFT journal (not posted) dated on the payment due date:
...
function jeNumberFor(cardAccountNumber, statementDate) {
...
export async function ensureCreditCardPaymentDue(db, {
  ...
  const dueDay = String(paymentDueDate || '').slice(0, 10);
  ...
  if (!(amt > 0.005) || !dueDay || !payFromAccountId) {
    ...
    return { skipped: true, reason: 'missing amount, due date, or pay-from account' };
  }
  ...
  const description = `Scheduled payment — ${cardLabel} (stmt ${stmtDay})`;
  ...
  if (!jeId) {
    ...
    await db.run(
      `INSERT INTO journal_entries
       ...
       VALUES (?, ?, ?, ?, ?, 'DRAFT', ?, ?)`,
      [jeId, entityId, jeNumber, dueDay, description, userId, CC_PAYMENT_DUE_SOURCE]
    );
  } else {
    await db.run(
      ...
      [dueDay, description, CC_PAYMENT_DUE_SOURCE, jeId, entityId]
    );
    await db.run(`DELETE FROM journal_entry_lines WHERE journal_entry_id = ?`, [jeId]);
  }

  // DR card liability (payment), CR cash
  await db.run(
