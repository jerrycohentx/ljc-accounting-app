...
function isGmailOAuthServerConfigured() {
...
/** Send email via Gmail API (requires gmail.send scope on refresh token). */
export async function sendGmailOAuthMail({ refreshToken, from, to, subject, text, html }) {
  if (!isGmailOAuthServerConfigured()) {
    throw new Error('Gmail OAuth not configured on server');
  }
  const token = await refreshAccessToken(refreshToken);
  const raw = buildRawEmail({ from: from || 'me', to, subject, text, html });
  ...
  return { id: data.id, from, to, transport: 'gmail-oauth' };
}
...
export async function* fetchGmailMessages({ user, refresh_token, sinceDays, searchQuery }) {
  if (!isGmailOAuthServerConfigured()) {
    throw new Error(
      'Gmail OAuth not configured on server — set GMAIL_OAUTH_CLIENT_ID and GMAIL_OAUTH_CLIENT_SECRET on Render'
    );
  }
  const token = await refreshAccessToken(refresh_token);
  const q = `${searchQuery} newer_than:${sinceDays}d`;
  const listUrl = `https://gmail.googleapis.com/gmail/v1/users/me/messages?q=${encodeURIComponent(q)}&maxResults=80&includeSpamTrash=true`;
