...
function toIsoDay(value) {
  ...
  // Only accept a real YYYY-MM-DD. A malformed statement date (e.g. "Mon Jan 05"
  // from a PDF parse) must never reach a Postgres DATE column, or the whole
  // worksheet load 500s. Returning '' lets callers fall back to a valid date.
  ...
  const persistPair = async (gl, bank, method) => {
    ...
    await db.run(
      ...
      [
        ...
        bank.id,
        glAmount,
        toIsoDay(bank.date) || toIsoDay(gl.posting_date) || new Date().toISOString().slice(0, 10),
        // matched_by is a NOT NULL FK to users(id); it must be a real user id,
        // not a "userId:method" composite (that violates the FK and 500s the load).
        userId,
