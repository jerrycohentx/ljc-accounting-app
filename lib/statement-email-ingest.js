...
async function extractPdfStatement(pdfPath) {
  ...
  try {
    const { stdout, stderr } = await execFileAsync('python3', [script, pdfPath], {
    ...
    if (/ENOENT|not found/i.test(detail)) {
      throw new Error('PDF parser unavailable on server (python3/pypdf). Contact support.');
    }
    if (/pypdf not installed/i.test(detail)) {
      throw new Error('PDF parser missing pypdf on server — redeploy after build installs requirements.txt');
    }
    throw new Error(`PDF extract failed: ${detail.slice(0, 200)}`);
  }
}
...
async function collectMessages(db, settings) {
  const messages = [];
...
export async function runStatementEmailIngest(db, { reason = 'scheduled', userId = 'system-email-ingest' } = {}) {
  ...
  try {
    ...
    const { messages, mailboxes: configuredMailboxes, mailboxStats } = await collectMessages(db, settings);
    ...
    if (reason === 'manual' && getLonestarPortalConfig().enabled && !hadLonestarDownload) {
      try {
        const r = await processLonestarPortalDirect(db, { settings, userId, reason: 'manual' });
        results.push({
