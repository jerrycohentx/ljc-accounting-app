...
async function processLonestarNotification(db, { email, settings, userId }) {
  ...
  const fileName = fetchResult.fileName || 'lonestar-estatement.pdf';
  const savedPath = saveInboxFile(
    settings,
    email.mailbox,
    email.messageId,
  ...
  const importResult = await importStatementForReconcile(db, {
    ...
    accountId: account.id,
    userId,
    pdfBase64: fetchResult.buffer.toString('base64'),
    fileName,
    autoPost: true,
  });
  ...
  return {
    ...
    mergeResult,
    importResult,
    downloadSource: fetchResult.source,
    notificationMeta: fetchResult.meta,
  };
}
...
async function collectMessages(db, settings) {
  ...
  const lonestarQuery = `from:lsbtexas.com OR from:info@lsbtexas.com OR subject:estatement`;
  ...
  for (const mb of mailboxes) {
    try {
      if (mb.transport === 'gmail-oauth') {
        ...
        for (const msg of imapMsgs) pushMsg(msg);
        await markMailboxSync(db, mb.user);
      }
    } catch (err) {
      await markMailboxSync(db, mb.user, { error: err.message });
...
export async function runStatementEmailIngest(db, { reason = 'scheduled', userId = 'system-email-ingest' } = {}) {
  ...
  try {
    ...
    const { messages, mailboxes: configuredMailboxes } = await collectMessages(db, settings);
