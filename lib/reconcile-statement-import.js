...
export async function importStatementForReconcile(db, {
  ...
  if (ofxContent) {
    ...
    // Cross-account guard for PDFs — the OFX branch has had this from the start,
    // but the PDF branch never compared, and the extractor ALREADY reads the
    // account number off the statement (meta.accountLast4). Without this, a
    // Simmons PDF uploaded while Lone Star was selected (the screen's default
    // account) landed 42 Simmons lines on Lone Star AND merged Simmons balances
    // into Lone Star's bundled statement JSON (2026-07-16). Must run BEFORE
    // mergeStatementJson — the merge is exactly what contaminates.
    const pdfLast4 = data.meta?.accountLast4 ? String(data.meta.accountLast4) : null;
    if (wantLast4 && pdfLast4 && pdfLast4 !== wantLast4) {
      throw new Error(
        `This statement is for account ****${pdfLast4}${data.meta?.bankName ? ` (${data.meta.bankName})` : ''}, `
        + `but you are reconciling ${account.account_name} (****${wantLast4}). `
        + 'Import canceled to prevent cross-account contamination — select the matching account first, then upload.'
      );
    }
