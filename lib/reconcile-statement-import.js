...
export async function importStatementForReconcile(db, {
  ...
  try {
    ...
    for (const r of glRows) {
      const key = `${String(r.d).slice(0, 10)}|${Math.round(Math.abs((Number(r.dr) || 0) - (Number(r.cr) || 0)) * 100)}`;
      booked.set(key, (booked.get(key) || 0) + 1);
    }
    const kept = [];
    for (const t of newTransactions) {
      const key = `${String(t.date || t.postingDate || '').slice(0, 10)}|${Math.round(Math.abs(Number(t.amount) || 0) * 100)}`;
