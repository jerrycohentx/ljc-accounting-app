...
import { checkAutoPostAllowed } from './auto-post-policy.js';
...
export async function importStatementForReconcile(db, {
  ...
  if (ofxContent) {
  ...
  } else if (parsedStatement?.transactions?.length) {
    ...
    if (jsonPathForAccount(account.account_number, entityId)) {
      try {
        mergeStatementJson(account.account_number, data, entityId);
      } catch (err) {
        console.warn('mergeStatementJson during parsed import (non-fatal):', err.message);
      }
    }
  } else if (pdfBase64) {
    ...
    try {
      ...
      // Fallback: Python extractors (Simmons checking, then Amex) when Node parse fails.
      const scripts = [
        path.join(ROOT, 'scripts', 'extract-simmons-pdf.py'),
        path.join(ROOT, 'scripts', 'extract-amex-pdf.py'),
      ];
      let lastErr = nodeErr;
      data = null;
      ...
      if (!data) throw new Error(`PDF extract failed: ${nodeErr.message}`);
      void lastErr;
    } finally {
      try {
        fs.unlinkSync(tmp);
  ...
  if (!createEntries) {
    ...
    // ---- ENTRY-CREATING path (email statement ingest) ----
    // Credit-card liabilities use the Amex/card commit path so payments never
    // invent CR Cash (bank OFX owns the cash leg).
    if (isLiabilityAccount(account) || ['2010', '2011', '2012', '2013'].includes(String(account.account_number))) {
      const cardResult = await commitAmexImportTransactions(db, {
        ...
        cardAccountNumber: String(account.account_number),
      ...
      await commitBankImportTransactions(db, {
        ...
        // Attach the uploaded statement itself to mortgage/loan-payment drafts.
        sourceDocument: pdfBase64
          ? { fileName, fileMime: 'application/pdf', fileData: pdfBase64 }
          : null,
      });
    ...
    if (autoPost) {
      const gate = checkAutoPostAllowed('reconcile statement import');
      if (!gate.allowed) {
        return {
          imported,
